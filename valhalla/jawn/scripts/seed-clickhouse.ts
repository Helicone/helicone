...
async function main() {
  ...
  console.log(
    `[seed-clickhouse] Seeding request_response_rmt with ${count} rows for orgId=${orgId}`,
  );
  ...
  const now = Date.now();
  const rows: RequestResponseRMT[] = Array.from({ length: count }).map(
    (_, i) => {
  ...
  const result = await testClickhouseDb.dbInsertClickhouse(
