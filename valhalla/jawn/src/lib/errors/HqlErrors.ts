...
export const HqlErrorMessages: Record<HqlErrorCode, string> = {
  ...
  [HqlErrorCode.QUERY_TIMEOUT]:
    "Query execution timeout (30 seconds). Please optimize your query.",
  [HqlErrorCode.MEMORY_LIMIT_EXCEEDED]:
    "Query exceeded memory limit. Please reduce the data scope.",
  [HqlErrorCode.ROW_LIMIT_EXCEEDED]:
    "Query attempted to read too many rows. Please add more filters.",
  [HqlErrorCode.RESULT_LIMIT_EXCEEDED]:
    "Query result exceeded maximum rows (10,000). Please add LIMIT clause.",
  ...
  [HqlErrorCode.QUERY_NAME_TOO_LONG]:
    "Query name must be less than 255 characters",

  [HqlErrorCode.CSV_UPLOAD_FAILED]: "Failed to upload CSV",
  [HqlErrorCode.CSV_URL_NOT_RETURNED]:
    "CSV upload succeeded but no URL was returned",

  [HqlErrorCode.FEATURE_NOT_ENABLED]:
    "Access to HQL feature is not enabled for your organization",
...
export function createHqlError(code: HqlErrorCode, details?: string): HqlError {
  return {
    code,
