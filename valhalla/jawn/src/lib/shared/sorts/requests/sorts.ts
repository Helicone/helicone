...
export function buildRequestSortClickhouse(sort: SortLeafRequest, baseTable: RequestResponseRMTDerivedTable = "request_response_rmt"): string {
  if (sort.random) {
    return "rand()";
  }
  if (sort.created_at) {
    assertValidSortDirection(sort.created_at);
    return `${baseTable}.request_created_at ${sort.created_at}`;
  }
  if (sort.latency) {
    assertValidSortDirection(sort.latency);
    return `${baseTable}.latency ${sort.latency}`;
  }
  if (sort.total_tokens) {
    assertValidSortDirection(sort.total_tokens);
    return `(${baseTable}.prompt_tokens + ${baseTable}.completion_tokens) ${sort.total_tokens}`;
  }
  if (sort.completion_tokens) {
    assertValidSortDirection(sort.completion_tokens);
    return `${baseTable}.completion_tokens ${sort.completion_tokens}`;
  }
  if (sort.prompt_tokens) {
    assertValidSortDirection(sort.prompt_tokens);
    return `${baseTable}.prompt_tokens ${sort.prompt_tokens}`;
  }
  if (sort.user_id) {
    assertValidSortDirection(sort.user_id);
    return `${baseTable}.user_id ${sort.user_id}`;
  }
  if (sort.body_model) {
    assertValidSortDirection(sort.body_model);
    return `${baseTable}.model ${sort.body_model}`;
  }
  if (sort.is_cached) {
    assertValidSortDirection(sort.is_cached);
    return `${baseTable}.is_cached ${sort.is_cached}`;
  }
  if (sort.request_prompt) {
    assertValidSortDirection(sort.request_prompt);
    return `JSONExtractString(${baseTable}.request_body, 'prompt') ${sort.request_prompt}`;
  }
  if (sort.response_text) {
    assertValidSortDirection(sort.response_text);
    return `JSONExtractString(${baseTable}.response_body, 'choices', 0, 'text') ${sort.response_text}`;
  }
  if (sort.cost_usd) {
    assertValidSortDirection(sort.cost_usd);
    return `cost_usd ${sort.cost_usd}`;
  ...
  if (sort.properties) {
    for (const key in sort.properties) {
      ...
      return `${baseTable}.properties['${key}'] ${sort.properties[key]}`;
    }
  }

  // Default sort if no valid sort option is provided
  return `${baseTable}.request_created_at DESC`;
}
