...
export function buildRequestSortClickhouse(sort: SortLeafRequest): string {
  ...
  if (sort.created_at) {
    assertValidSortDirection(sort.created_at);
    return `request_response_versioned.request_created_at ${sort.created_at}`;
  }
  if (sort.latency) {
    assertValidSortDirection(sort.latency);
    return `request_response_versioned.latency ${sort.latency}`;
  }
  if (sort.total_tokens) {
    assertValidSortDirection(sort.total_tokens);
    return `(request_response_versioned.prompt_tokens + request_response_versioned.completion_tokens) ${sort.total_tokens}`;
  }
  if (sort.completion_tokens) {
    assertValidSortDirection(sort.completion_tokens);
    return `request_response_versioned.completion_tokens ${sort.completion_tokens}`;
  }
  if (sort.prompt_tokens) {
    assertValidSortDirection(sort.prompt_tokens);
    return `request_response_versioned.prompt_tokens ${sort.prompt_tokens}`;
  }
  if (sort.user_id) {
    assertValidSortDirection(sort.user_id);
    return `request_response_versioned.user_id ${sort.user_id}`;
  }
  if (sort.body_model) {
    assertValidSortDirection(sort.body_model);
    return `request_response_versioned.model ${sort.body_model}`;
  }
  if (sort.is_cached) {
    assertValidSortDirection(sort.is_cached);
    return `request_response_versioned.is_cached ${sort.is_cached}`;
  }
  if (sort.request_prompt) {
    assertValidSortDirection(sort.request_prompt);
    return `JSONExtractString(request_response_versioned.request_body, 'prompt') ${sort.request_prompt}`;
  }
  if (sort.response_text) {
    assertValidSortDirection(sort.response_text);
    return `JSONExtractString(request_response_versioned.response_body, 'choices', 0, 'text') ${sort.response_text}`;
  }
  if (sort.properties) {
    for (const key in sort.properties) {
      assertValidSortDirection(sort.properties[key]);
      ...
      return `request_response_versioned.properties['${key}'] ${sort.properties[key]}`;
    }
  }

  // Default sort if no valid sort option is provided
  return `request_response_versioned.request_created_at DESC`;
}
