...
The AI Gateway ships with comprehensive built-in configurations for all major LLM providers, including their endpoints, API versions, and supported models. **Most users never need to configure providers manually.**
...
The gateway includes [comprehensive defaults](https://github.com/Helicone/helicone-router/blob/main/llm-proxy/config/embedded/providers.yaml) that work out-of-the-box for standard deployments.
...
<Note>
  **New models are automatically detected!** The router uses smart model detection with embedded configurations that are regularly updated. You don't need to manually add new models unless you want to restrict the available model set.
</Note>

<Note>
  Provider configuration only defines endpoints and models. API key management is handled separately in [Secret Management](/gateway/concepts/secret-management).
</Note>
...
## Reference
<Note>
  For complete configuration options and syntax, see the [Configuration Reference](/gateway/config#provider-configuration).
</Note>
...
| **Google Gemini** | `https://generativelanguage.googleapis.com` | ✅ (VertexAI, custom)   |
...
For current model support by provider, see the [embedded provider configuration](https://github.com/Helicone/helicone-router/blob/main/llm-proxy/config/embedded/providers.yaml) which is automatically updated as new models are released.
...
Override default provider endpoints for enterprise deployments, regional compliance, or self-hosted models:

```yaml
providers:
  ollama:
    base-url: "http://192.168.1.100:8080"  # Custom Ollama server
  
  bedrock:
    base-url: "https://bedrock-runtime.us-west-2.amazonaws.com"  # Regional AWS endpoint
  
  anthropic:
    version: "2023-06-01"  # Lock API version
```

See [Configuration Reference](/gateway/config#provider-configuration) for complete router configurations.