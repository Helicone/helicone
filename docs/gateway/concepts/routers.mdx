...
<Note>
  For complete configuration options and syntax, see the [Configuration Reference](/gateway/config#router-configuration).
</Note>
...
<ParamField path="load-balance" type="object" required>
  ...
  [Learn more about load balancing →](/gateway/features/loadbalancing)
</ParamField>
...
<ParamField path="cache" type="object" optional>
  ...
  [Learn more about caching →](/gateway/features/cache)
</ParamField>
...
<ParamField path="retries" type="object" optional>
  ...
  [Learn more about retries →](/gateway/features/retries)
</ParamField>
...
<ParamField path="rate-limit" type="object" optional>
  ...
  [Learn more about rate limiting →](/gateway/features/rate-limiting)
</ParamField>
...
<ParamField path="model-mappings" type="object" optional>
  ...
  [Learn more about model mapping →](/gateway/features/model-mapping)
</ParamField>
...
routers:
  default:
    load-balance:
      chat:
        strategy: latency
        targets: []  # No providers configured by default
```
    ...
    load-balance:
      chat:
        ...
        targets:
          ...
          - gemini      # Will be removed if unhealthy
```
...
<Note>
  **Dynamic Router Management:** Hot-reloading router configurations and API-driven router management coming in v2.
</Note>

- **Runtime router creation** - Add/remove routers without restarting
- **Router templates** - Predefined router configurations for common patterns  
- **Router metrics** - Per-router observability and performance metrics
- **Conditional routing** - Route based on request headers, user attributes, or other conditions 