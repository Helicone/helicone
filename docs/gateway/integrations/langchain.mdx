...
const chat = new ChatOpenAI({
    ...
    configuration: {
        baseURL: "https://ai-gateway.helicone.ai/"  // Add this!
    },
});
```
...
// Helicone-configured model instance
const model = new ChatOpenAI({
  modelName: "claude-3-haiku-20240307", // any supported model via Helicone
  apiKey: process.env.HELICONE_API_KEY,
  configuration: {
    baseURL: "https://ai-gateway.helicone.ai/v1",
    defaultHeaders: {
      "Helicone-Session-Id": "agent-session-001",
      "Helicone-User-Id": "User1234",
      "Helicone-Property-Environment": "production",
    },
  },
});
...
// Agent with Helicone-backed model
...
const result = await agent.invoke({
  messages: [
    { role: "user", content: "Search AI news and summarize top 3 stories." },
  ],
});
...
# Helicone-configured model instance
model = ChatOpenAI(
    model="claude-3-haiku-20240307",  # any supported model via Helicone AI Gateway
    api_key=os.getenv("HELICONE_API_KEY"),
    base_url="https://ai-gateway.helicone.ai/v1",
    default_headers={
        "Helicone-Session-Id": "agent-session-001",
...
# Create agent with simple system prompt string
agent = create_agent(
    model,
    tools,
...
# Invoke
result = agent.invoke({
    "messages": [{"role": "user", "content": "Search AI news and summarize top 3 stories."}]
})
