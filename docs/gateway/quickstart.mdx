...
<Steps>
    <Step title="Configure provider secrets">
        ...
        The default configuration for the Gateway enables OpenAI as the only
        provider to make quickstart easy.
    ...
    The Gateway will be running on `http://localhost:5678` and has
    three routes:
    - `/ai` for a Unified API endpoint compatible with the OpenAI endpoint
    - `/router/{router-name}` for load balancing
    ...
    const openai = new OpenAI({
      baseURL: "http://localhost:5678/ai"
    });
    ...
    openai.api_base = "http://localhost:5678"
    ...
    ```
    ```bash Bash
    curl http://localhost:5678/ai/v1/chat/completions \
    ...
    ```bash
    export AI_GATEWAY__HELICONE__API_KEY=your-api-key
    ```
    </Step>
</Steps>
