...
<Tabs>
  ...
  <Tab title="Provider Ignore">
    **Format:** `model: "!providerName,gpt-4o-mini"`

    **Best for:**
    - Excluding specific providers from automatic routing
    - Avoiding providers with known issues
    - Testing without certain providers
    - Compliance restrictions against specific providers

    **How it works:**
    Use the `!` prefix to exclude specific providers from automatic routing. The gateway will try all other available providers except those you've ignored. You can ignore multiple providers by chaining them with commas.

    **Examples:**
    ```typescript
    // Ignore a single provider
    ...
    **Example scenario:**
    You know OpenAI is having issues today, so you want automatic routing but specifically avoid OpenAI: `"!openai,gpt-4o-mini"`. The gateway tries Azure, AWS Bedrock, and other providers instead.
  </Tab>
</Tabs>
