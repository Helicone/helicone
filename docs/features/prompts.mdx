...
<Frame caption="Example: A Prompt Template Designed for a Rap Battle">
  <img src="/images/prompts/example-rap-battle.png" />
</Frame>

### Why Prompts
With the introduction of prompts, you can now create **prompt templates**. Requests are now matched to a `helicone-prompt-id`, allowing you to: 
...
<Note>Not sure if proxy is for you? We created a guide to explain the difference between [Helicone Proxy vs Helicone Async integration](https://docs.helicone.ai/getting-started/proxy-vs-async).</Note>
...
<Accordion title="Get a Deep-Dive of Prompt Templates">

### For Typescript / Javascript users:

By prefixing your prompt with `hprompt` and enclosing your input variables in an additional `{}`, it allows Helicone to easily detect your prompt and inputs. We wanted to make it as easy as possible to use Prompts, **and that means minimum code change**. 
...
<Tabs>
  ...
  <Steps>
    ...
    ```    
    <Warning>It is crucial that each key has a **unique identifier**; otherwise, the same variable will be replaced in multiple places. </Warning>
