...
Once you've created sessions, you can query and filter them using the [Session Query API](/rest/session/post-v1sessionquery). This is essential for debugging, analytics, and building custom dashboards.
...
<Tabs>
<Tab title="Last 24 Hours by User">
...
<Tab title="Debug Failed Sessions">
Find sessions with error status:
...
<Tab title="Production Sessions Only">
...
Once you have a session, retrieve all its requests using the [Request Query API](/rest/request/post-v1requestquery):
...
<Note>
  For detailed information on querying requests, see the [Request Query API documentation](/rest/request/post-v1requestquery-clickhouse).
</Note>

## Related Features

<CardGroup cols={2}>
<Card title="Session Query API" icon="code" href="/rest/session/post-v1sessionquery">
