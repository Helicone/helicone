...
For TypeScript users, we provide two ways to use Helicone prompts:
...
For cases where you need to access the prompt body directly, the helpers SDK provides the `HeliconePromptManager`:
...
The `HeliconePromptManager` provides two main methods:

| Method | Description | Returns |
|--------|-------------|---------|
| `pullPromptBody()` | Fetch raw prompt from storage | `ChatCompletionCreateParams` |
| `getPromptBody()` | Compile prompt with inputs and validation | `{ body: ChatCompletionCreateParams, errors: ValidationError[] }` |
...
<Tabs>
  ...
  <Tab title="With Version Control">
    ```typescript
    import OpenAI from 'openai';
    import { HeliconePromptManager } from '@helicone/helpers';
