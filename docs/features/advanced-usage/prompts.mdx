...
Every helper accepts an optional `useCache` boolean as the final argument. Pass `true` when you're calling prompts repeatedly from a trusted backend and can tolerate up to ~5 minutes of staleness; omit it for always-fresh UI requests.
    ...
    <CodeGroup>
      ...
      async function generateWithPrompt() {
        ...
        const { body, errors } = await promptManager.getPromptBody(
          ...
          true // optional: use Helicone's cached endpoint for repeat lookups
        );
