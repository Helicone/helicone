...
| `Cache-Control` (optional)                  | Allow you to configure cache limit based on the [Cloudflare Cache Directive](https://developers.cloudflare.com/cache/about/cache-control#cache-control-directives). Currently only max-age is supported, but we will be adding more configuration options soon. |
| `Helicone-Cache-Bucket-Max-Size` (optional) | Allows you to choose how large you want your Cache Bucket size to be.|
| `Helicone-Cache-Seed` (optional) | Allows you to generate for predictable results and maintain separate cache states for each cache seed. | 
