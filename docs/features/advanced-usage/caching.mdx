...
| `Helicone-Cache-Bucket-Max-Size` (optional) | Configure your Cache Bucket size, represented as a `string`. |
| `Helicone-Cache-Seed` (optional) | A `string` that defines a separate cache state to generate predictable results, i.e. `user-123`. |
