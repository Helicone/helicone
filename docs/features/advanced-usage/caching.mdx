...
```typescript
// Hash the code snippet to create consistent cache keys
const codeHash = crypto.createHash('md5').update(codeSnippet).digest('hex');
...
const response = await openai.chat.completions.create(
  ...
  },
  {
    headers: {
      "Helicone-Cache-Enabled": "true",
      "Helicone-Cache-Seed": codeHash,    // Same code = same cache
...
- **Model name**: `gpt-4` vs `gpt-3.5-turbo` 
- **Messages**: Exact message content and order
- **Parameters**: Temperature, max_tokens, etc.
- **Cache seed**: If specified

**What triggers cache hits:**
```typescript
// ✅ Cache hit - identical requests
const request1 = { model: "gpt-4", messages: [{ role: "user", content: "Hello" }] };
const request2 = { model: "gpt-4", messages: [{ role: "user", content: "Hello" }] };

// ❌ Cache miss - different content  
const request3 = { model: "gpt-4", messages: [{ role: "user", content: "Hi" }] };

// ❌ Cache miss - different parameters
const request4 = { model: "gpt-4", messages: [{ role: "user", content: "Hello" }], temperature: 0.5 };
```
...
- **Storage location**: Cached on Cloudflare edge, not your infrastructure
