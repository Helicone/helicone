...
- `unit` (optional): must be either `request` or `cents`. This will change your rate limit to be by cents (cost) or request (count)
...
Example: `10;w=1000;u=cents;s=user`
This policy allows 10 cents of requests per 1000 seconds per user.
