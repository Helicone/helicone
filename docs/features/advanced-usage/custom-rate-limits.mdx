...
| `time_window` (required)             | The unit is **seconds**. For example, you would use `w=86400` (60 * 60 * 24 = 86400) to set the time window for a single day. The minimum is `60` seconds.     |
...
<Note>If a request is rate-limited, a 429 rate limit error will be returned.</Note>
...
<Note>Contact [help@helicone.ai](help@helicone.ai) if you have any questions.</Note>
