...
<Note>
  Helicone doesn't run evaluations for you - we're not an evaluation framework.
  Instead, we provide a centralized location to report and analyze evaluation
  results from any framework (like RAGAS, LangSmith, or custom evaluations),
  giving you unified observability across all your evaluation metrics.
</Note>
...
<Warning>
  Scores are processed with a **10 minute delay** by default for analytics
  aggregation.
</Warning>
...
def evaluate_rag_response(question, answer, contexts, ground_truth, requestId): # Initialize RAGAS metrics
