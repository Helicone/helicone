...
Control webhook volume by setting a sample rate:

```yaml
# Send webhooks for 25% of requests
sample_rate: 25

# Send webhooks for all requests
sample_rate: 100
...
<Tip>Use lower sample rates for high-volume applications to reduce webhook processing load.</Tip>
</Accordion>
</AccordionGroup>
...
- Fetch data promptly after receiving webhooks
...
- Consider payload size when processing webhooks

### Setting User IDs

To include `user_id` in webhook payloads, set it in your LLM requests:

```javascript
// Option 1: Using Helicone-User-Id header
const response = await openai.chat.completions.create(
  { /* request data */ },
  {
    headers: {
      "Helicone-Auth": "Bearer YOUR_API_KEY",
      "Helicone-User-Id": "user-123" // Included in webhooks
    }
  }
);
...
const response = await openai.chat.completions.create(
  { /* request data */ },
  {
    headers: {
      "Helicone-Auth": "Bearer YOUR_API_KEY", 
      "Helicone-Property-UserId": "user-123" // Also included
    }
  }
);
```

<Note>The `user_id` field only appears in webhook payloads when explicitly set in the original request.</Note>
