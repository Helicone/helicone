...
<Warning>
  **Important Notice:** As of April 25th, 2025, active development on the `@helicone/generate` API is paused. We are focusing our efforts on a new router project which will include an improved version of this API, scheduled to launch around June 1st, 2025.

We will provide announcements and ensure smooth migration paths. The current `@helicone/generate` API will continue to function until May 1st, 2026.
