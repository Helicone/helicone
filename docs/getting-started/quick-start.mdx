...
<Step title="Create an account">
  ...
  <Step title="Integrate with AI Gateway (Recommended)">

    The fastest way to get started is with Helicone's AI Gateway - use OpenAI SDK to access models from OpenAI, Anthropic, Google, AWS Bedrock, and 100+ other providers through a single unified API.

    **Set up provider keys:** Go to [Provider Keys](https://us.helicone.ai/providers) to securely store API keys for the providers you want to use.
    ...
    <Note>
      **Why AI Gateway?** One SDK for all providers, automatic fallbacks (`"gpt-4o/openai,claude-3.5-sonnet-v2/anthropic"`), prompt management, and unified observability. See all [supported models →](/ai-gateway/model-registry)
    </Note>
  </Step>
...
Need provider-specific features or using existing SDKs? Explore [all integration options →](/integrations/overview)

<CardGroup cols={1}>
<Card title="When to use alternatives" icon="question">
  Use direct provider integrations if you need specific features only available through native SDKs, have existing code deeply integrated with a provider, or have enterprise requirements that prevent using the unified API.
</Card>
</CardGroup>
