...
<Tabs>
  ...
  <Tab title="Node.js">
    ...
    const stream = await client.chat.completions.create({
      ...
      max_tokens: 2500,
      stream: true,
      // TrueFoundry metadata
      extra_headers: {
        "X-TFY-METADATA": JSON.stringify({ tfy_log_request: true }),
      },
    });
  ...
  <Tab title="cURL">
    ```bash
    # Direct HTTP request to TrueFoundry AI Gateway
    ...
    **Note**: HeliconeAsyncLogger requires a runtime environment (Python/Node.js) for async logging. For cURL users who need Helicone logging, consider:
    - Using the Python or Node.js examples above, or
    - Running HeliconeAsyncLogger in a separate service to capture HTTP requests
  </Tab>
</Tabs>
