...
<Tabs>
  ...
  <Tab title="cURL">
    ```bash
    # First install helicone-async (Python example)
    pip install helicone-async openai

    # Then use with TrueFoundry gateway
    ...
    Note: For cURL, you'll need to use the Python or Node.js HeliconeAsyncLogger in a separate process to capture and log the requests to Helicone.
  </Tab>
</Tabs>
