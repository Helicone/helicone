...
The all-in-one image runs the dashboard and the Jawn API only. Jawn's
`/v1/gateway/*` proxy routes have been removed, so LLM requests must go through
the [AI Gateway](/gateway/overview), which you can deploy next to this image.
