...
<Steps>
  ...
  <Step title={strings.configureWebSocketConnection}>
    ...
    <CodeGroup>
      ...
      const url = "wss://api.helicone.ai/v1/ai-gateway/oai/realtime?model=[MODEL_NAME]"; // gpt-4o-realtime-preview-2024-12-17
      ...
      const url = `wss://api.helicone.ai/v1/ai-gateway/oai/realtime?resource=${process.env.AZURE_RESOURCE}&deployment=${process.env.AZURE_DEPLOYMENT}`;
