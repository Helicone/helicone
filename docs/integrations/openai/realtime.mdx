...
<Warning>
  **This integration has been removed.** The Helicone Realtime WebSocket proxy at
  `wss://api.helicone.ai/v1/gateway/oai/realtime` is no longer served, and the
  instructions below will not work. Use the [AI Gateway](/gateway/overview) for
  LLM proxying and observability. This page is kept for reference only.
</Warning>
