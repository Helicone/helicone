...
<ParamField header="Helicone-Target-URL" type="string(url)" required>
The URL to proxy the request to when using gateway.hconeai.com. 
For example, https://api.openai.com/v1/engines/davinci/completions
</ParamField>

<ParamField header="Helicone-OpenAI-Api-Base" type="string(url)">
The URL to proxy the request to when using oai.hconeai.com. 
For example, https://[YOUR_AZURE_DOMAIN].openai.azure.com. 
</ParamField>
