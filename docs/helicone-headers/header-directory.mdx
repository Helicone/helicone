...
<ResponseField name="Helicone-Cache-Enabled" type="boolean">
  ...
  <Accordion title="Additional headers" defaultOpen>
    ...
    | `Helicone-Cache-Bucket-Max-Size`  | The size of cache bucket represented as a `string`.        |
    | `Helicone-Cache-Seed`             | A `string` that defines a separate cache state to generate predictable results, i.e. `user-123`. |
  </Accordion>
</ResponseField>
<ResponseField name="Helicone-Retry-Enabled" type="boolean">
  ...
  <Accordion title="Additional headers" defaultOpen>
    ...
    <Note> The type for all retry parameters is a number represented as a `string`.</Note> 
  </Accordion>
</ResponseField>
