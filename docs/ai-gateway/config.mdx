...
Automatically retry transient errors with a customizable retry policy.
...
<ParamField path="global.rate-limit" type="object">
  ...
  Available strategies are `constant`:
...
Configure a more specific retry configuration at the granularity of each router

<ParamField path="routers.[name].retries" type="object">
  Configure an retry policy that applies to requests to a specific router.
