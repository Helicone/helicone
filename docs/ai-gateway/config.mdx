...
<ParamField path="routers.[name].cache.enabled" type="boolean">
  ...
  **How it works:** When enabled, the Gateway will cache responses for identical requests. Optionally override with `Helicone-Cache-Enabled: false` request header.
</ParamField>
...
<ParamField path="telemetry.level" type="string" default="info">
  ...
  **Common patterns:**
  ...
  - `"debug"` - Debug for dependencies, info for gateway
  - `"warn"` - Warnings only, disable specific modules
</ParamField>
