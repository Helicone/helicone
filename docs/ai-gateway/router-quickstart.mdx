...
### Step 3: Environment-Specific Rate Limits

Real applications need different rate limits for different environments. Let's create production and development routers with appropriate protections:
...
routers:
  ...
  staging:
    ...
    cache:
      store:
        in-memory: {}
      directive: "no-cache"  # No caching for testing
```
...
| **staging** | `/router/staging...` | 200/min | Testing without cache interference |
...
<Steps>
  ...
  <Step title="Test rate limits">
    ```bash
    # Production router - allows 1000 requests per minute
    ...
    # Development router - only allows 100 requests per hour
    curl -X POST http://localhost:8080/router/development/v1/chat/completions \
      -H "Content-Type: application/json" \
      -d '{"model": "gpt-4o-mini", "messages": [{"role": "user", "content": "Development test"}]}'
    
    # Test rate limiting by making multiple quick requests to development:
    for i in {1..5}; do
      curl -X POST http://localhost:8080/router/development/v1/chat/completions \
        -H "Content-Type: application/json" \
        -d '{"model": "gpt-4o-mini", "messages": [{"role": "user", "content": "Request '$i'"}]}'
      echo "Request $i completed"
    done
    ```
    
    <Note>
      The development router will start rate limiting after you exceed the hourly quota, returning HTTP 429 responses.
    </Note>
  </Step>
</Steps>
...
Just change the base URL to use different routers:
