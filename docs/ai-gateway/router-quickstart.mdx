...
<Note>
  **Prerequisites:** Make sure you've completed the [main
  quickstart](/ai-gateway/quickstart) and have the gateway running with your API
  keys configured.
</Note>
...
<Note>
  **Authentication Required**: Rate limiting is applied per-API-key, so you need
  Helicone authentication enabled to track and limit requests for different
  users.
</Note>
...
<CardGroup cols={2}>
  <Card
    ...
    icon="scale-balanced"
    href="/ai-gateway/concepts/loadbalancing"
  />
  <Card
    title="Intelligent Caching"
    icon="database"
    href="/ai-gateway/concepts/cache"
  />
  <Card
    title="Multiple Routers"
    icon="server"
    href="/ai-gateway/concepts/routers"
  />
  <Card
    title="Rate Limiting"
    icon="gauge-high"
    href="/ai-gateway/concepts/rate-limiting"
  />
</CardGroup>
...
<Card
  ...
  icon="rocket"
  href="/ai-gateway/deployment/overview"
  horizontal
>
