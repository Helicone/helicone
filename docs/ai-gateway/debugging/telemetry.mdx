...
The AI Gateway uses OpenTelemetry to automatically capture telemetry data from every request and export it to your observability backend.

**Data flow:** Request processing → Automatic instrumentation → Export to backend → Dashboard visualization

<Steps>
  <Step title="Request Instrumentation">
    Every request is automatically traced with spans for authentication, load balancing, provider calls, and response processing
  </Step>
  
  <Step title="Metrics Collection">
    Performance metrics, error rates, and system health data are continuously collected and exported
  </Step>
  
  <Step title="Structured Logging">
    All logs include trace correlation IDs and structured data for easy searching and analysis
  </Step>
  
  <Step title="Backend Export">
    Telemetry data is exported via OpenTelemetry protocol to your chosen observability platform
  </Step>
</Steps>
