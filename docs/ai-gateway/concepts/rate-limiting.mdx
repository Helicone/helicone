...
<Info>
  **Provider rate limits are handled automatically** by the load balancing
  system. This rate limiting feature is for controlling your own API traffic
...
<Note>
  ...
  Reference](/ai-gateway/config#rate-limiting).
</Note>
...
<AccordionGroup>
  <Accordion title="Per-API-Key Rate Limiting - Default" icon="key">
    **GCRA-based token bucket with burst capacity** Each API key gets a virtual
    token bucket with specified capacity. Requests consume tokens, which refill
    at the specified rate. Uses GCRA (Generic Cell Rate Algorithm) for smooth
    sustained rates with burst allowance. **Best for:** Preventing API key abuse
    while allowing reasonable burst traffic **How it works:** 1. Each API key
    gets a token bucket with specified capacity 2. Requests consume tokens from
    the bucket 3. Tokens refill at the specified rate (capacity /
    refill-frequency) 4. Requests are allowed if tokens are available, rejected
    otherwise **Example:** ```yaml global: rate-limit: per-api-key: capacity:
    500 refill-frequency: 1m # 500 requests per minute sustained ```
  </Accordion>
</AccordionGroup>

<Note>
  Additional rate limiting strategies (Per-End-User, Per-Team, Spend Limits,
  Usage Limits) are [coming soon](#coming-soon) for more granular control.
</Note>
...
<Step title="Rate Limit Check">
  The gateway checks rate limits in precedence order: - **Global rate limits**:
  Application-wide limits checked first - **Router-specific rate limits**:
  Individual router limits checked second
</Step>
