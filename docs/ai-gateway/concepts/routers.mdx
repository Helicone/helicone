...
<Note>
  ...
  Reference](/ai-gateway/config#router-configuration).
</Note>
...
<AccordionGroup>
  <Accordion title="Router-Based Routing (Recommended)">
    **Pattern:** `/router/{name}/{api - path}` **Examples:** -
    `/router/production/chat/completions` -
    `/router/development/chat/completions` - `/router/cost-optimized/embeddings`
    **Features:** - Full router configuration support - Load balancing, caching,
    retries - Router-specific rate limiting - Model mapping
  </Accordion>

<Accordion title="Direct Provider Routing">
  **Pattern:** `/{provider}/{api - path}` **Examples:** -
  `/openai/chat/completions` - `/anthropic/messages` -
  `/gemini/v1beta/generateContent` **Features:** - Direct proxy to specific
  provider - No load balancing or router features - Global rate limiting only
</Accordion>

  <Accordion title="Unified API Routing">
    **Pattern:** `/ai/{api - path}` **Examples:** - `/ai/chat/completions`
    **Features:** - Automatic provider detection from model name - Limited to
    OpenAI-compatible endpoints - No router-specific configuration
  </Accordion>
</AccordionGroup>
