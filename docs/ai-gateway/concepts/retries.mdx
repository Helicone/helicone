...
<Info>
  **Rate limit handling is automatic** - when providers return 429 status codes,
  the AI Gateway automatically removes them from load balancing rotation.
  Retries are for handling other types of failures like 5xx errors or network
  issues.
</Info>
...
<Note>
  ...
  Reference](/ai-gateway/config#retries).
</Note>
...
<Step title="Wait Period">
  AI Gateway waits for the calculated backoff period based on the configured
  strategy
</Step>
