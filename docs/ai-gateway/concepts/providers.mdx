...
Providers are enabled by providing credentials via environment variables. Many users can end here
as the gateway includes [comprehensive defaults](https://github.com/Helicone/helicone-router/blob/main/llm-proxy/config/embedded/providers.yaml)
that work out-of-the-box for standard deployments.
...
<Note>
  **New models are added immediately after their release!** The router ships with embedded configurations that are regularly updated. You don't need to manually configure new models unless you want to restrict the available model set.
</Note>
...
See [Configuration Reference](/ai-gateway/config#provider-configuration) for complete router configurations.