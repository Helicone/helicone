...
<Note>
  Provider configuration only defines endpoints and models. API key management
  is handled separately in [Secret
  Management](/ai-gateway/concepts/secret-management).
</Note>
...
<Note>
  ...
  Reference](/ai-gateway/config#provider-configuration).
</Note>
