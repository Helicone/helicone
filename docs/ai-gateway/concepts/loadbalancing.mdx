...
The AI Gateway currently supports load balancing across different AI providers and their regional deployments. This allows you to distribute traffic across different AI companies (OpenAI, Anthropic, Google) and their available regions for optimal performance and reliability.

**Examples:**
- **Providers**: OpenAI vs Anthropic vs Google Gemini
- **Deployments/Regions**: `us-east-1` vs `us-west-2` vs `eu-west-1`

<Note>
  Model-level load balancing (provider+model pairs) is [coming soon](#coming-soon) for more granular control.
</Note>
...
| **A/B testing**          | Weighted                   | v0           |
| **Compliance routing**   | [Multiple AI Gateways](#compliance-based-routing) | v0 |

<Note>
  Advanced strategies (Cost-optimized, Model-level weighted, Tag-based routing) are [coming soon](#coming-soon) for more sophisticated routing scenarios.
</Note>
