...
<Steps>
  ...
  <Step title="Convert to Helicone AI Gateway configuration">
      ...
      default:
        load-balance:
          chat:
            strategy: latency  
            providers:
              - openai    # Maps to your Azure deployment
              - anthropic # Direct provider support
    ...
    # If using custom endpoints (Azure, etc)
    providers:
      openai:
        base-url: "https://my-endpoint.openai.azure.com/"
        models:
          - gpt-35-turbo  # Azure model name

    # Set the port to LiteLLM's default port (optional)
    server:
        port: 4000
    ```

    Key mappings:
    - `master_key` → Helicone authentication
  ...
  <Step title="Set environment variables">
    ...
    # New: Helicone authentication (replaces master_key)
  ...
  <Step title="Migrate advanced features">
    ...
    **Virtual Keys → Helicone API Keys**
    ...
    LiteLLM virtual keys map directly to Helicone API keys. Instead of generating keys via API:
    ```bash
    # LiteLLM approach
    ...
    Use Helicone's dashboard at https://us.helicone.ai/settings/api-keys

    **Database/Spend Tracking → Built-in Observability**
    ...
    **Caching Configuration**
    ...
    If using LiteLLM's Redis cache, migrate to Helicone's in-memory caching:
    ```yaml
    routers:
      default:
        cache:
    ...
    ```
     ...
     <Note>
      AI Gateway will soon support Redis caching.
    </Note>
  </Step>
</Steps>
...
**Missing spend tracking:**
- Access Helicone dashboard for automatic tracking
