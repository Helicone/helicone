import { ok, err } from "../util/results";
export class RateLimitManager {
    async getRateLimitOptionsForKey(dbWrapper, userId, heliconeProperties) {
        try {
            // 1. Fetch policies
            const allPoliciesResult = await dbWrapper.getAllRateLimitPolicies();
            if (allPoliciesResult.error !== null || !allPoliciesResult.data) {
                return err(`Failed to retrieve rate limit policies: ${allPoliciesResult.error || "No data returned"}`);
            }
            ...
            if (requestPropertyKeys.length > 0) {
                const matchingPropertyPolicies = allPolicies.filter((policy) => policy.segment &&
                    policy.segment !== "user" &&
                    requestPropertyKeys.includes(policy.segment));
                if (matchingPropertyPolicies.length > 0) {
                    selectedPolicy = this.findMostRestrictivePolicy(matchingPropertyPolicies);
                    if (selectedPolicy) {
            ...
            if (userId) {
                const matchingUserPolicies = allPolicies.filter((policy) => policy.segment === "user");
            ...
            return err(`Exception selecting rate limit policy: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    formatPolicyToOptions(policy) {
        return ok({
            quota: policy.quota,
    ...
    findMostRestrictivePolicy(policies) {
        if (!policies || policies.length === 0) {
            return null;
        }
        // Calculate RPM for a policy - lower is more restrictive
        const calculateRPM = (policy) => {
        ...
        return policies
            ...
            .reduce((mostRestrictive, current) => current.rpm < mostRestrictive.rpm ? current : mostRestrictive, { policy: policies[0], rpm: calculateRPM(policies[0]) }).policy;
    }
}
