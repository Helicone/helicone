...
export class ReportManager {
    ...
    env;
    resendApiKey;
    constructor(reportStore, env) {
        this.reportStore = reportStore;
        this.env = env;
        this.resendApiKey = env.RESEND_API_KEY;
    }
    async sendReports() {
        const { data: allReports, error: allReportsErr } = await this.reportStore.getReports();
        if (allReportsErr) {
    ...
    async sendReportEmails(reports) {
        const promises = reports.map(async (report) => {
            const settings = report.settings;
            if (settings.emails.length > 0) {
                const { error: emailResErr } = await this.sendReportEmail(report, settings.emails);
                if (emailResErr) {
    ...
    async sendReportEmail(report, emails) {
        ...
        if (!res.ok) {
            return err(`Error sending emails: ${res.status} ${res.statusText} ${await res.text()}`);
        }
        return ok(null);
    }
    async sendReportSlacks(reports) {
        const promises = reports.map(async (report) => {
            const settings = report.settings;
            if (settings.slack_channels.length > 0) {
                const { error: slackResErr } = await this.sendReportSlack(report, settings.slack_channels);
                if (slackResErr) {
    ...
    async sendReportSlack(report, slack_channels) {
        const { slack_json } = await this.formatReportNotification(report);
        for (const channel of slack_channels) {
            const slackIntegration = report.organization.integrations.filter((integration) => integration.integration_name === "slack")[0];
            if (!slackIntegration) {
                return err(`Slack integration not found for report ${report.integration_name}`);
            }
            const settings = slackIntegration.settings;
            if (!settings.access_token) {
                return err(`Slack access token not found for organization ${report.organization.name}`);
            }
            ...
            if (!res.ok) {
                return err(`Error sending slack messages: ${res.status} ${res.statusText} ${await res.text()}`);
            }
    ...
    async formatReportNotification(report) {
        const subject = `Report`;
        const ONE_WEEK_MS = 1000 * 60 * 60 * 24 * 7;
        const [cost, numberOfRequests, errorRate, numberOfUsers, numberOfThreats, numberOfSessions, avgCostOfSessions,] = await Promise.all([
            ...
            this.reportStore.getAvgCostOfSessions(report.organization_id, ONE_WEEK_MS),
        ]);
        const EMOJIS = {
                            ...
                            elements: [
                                {
                                    ...
                                    elements: [
                                        ...
                                        {
                                            type: "emoji",
                                            name: `${EMOJIS[this.formatChangeData(cost.data?.value || 0, cost.data?.previous || 0)[1]]}`,
                                        },
                                        {
                                            type: "text",
                                            text: ` ${this.formatChangeData(cost.data?.value || 0, cost.data?.previous || 0)[0]})`,
                                        },
                                    ],
                                },
                                {
                                    ...
                                    elements: [
                                        ...
                                        {
                                            type: "emoji",
                                            name: `${EMOJIS[this.formatChangeData(numberOfRequests.data?.value || 0, numberOfRequests.data?.previous || 0)[1]]}`,
                                        },
                                        {
                                            type: "text",
                                            text: ` ${this.formatChangeData(numberOfRequests.data?.value || 0, numberOfRequests.data?.previous || 0)[0]})`,
                                        },
                                    ],
                                },
                                {
                                    ...
                                    elements: [
                                        ...
                                        {
                                            type: "emoji",
                                            name: `${EMOJIS[this.formatChangeData(errorRate.data?.value || 0, errorRate.data?.previous || 0, true)[1]]}`,
                                        },
                                        {
                                            type: "text",
                                            text: ` ${this.formatChangeData(errorRate.data?.value || 0, errorRate.data?.previous || 0)[0]})`,
                                        },
                                    ],
                                },
                                {
                                    ...
                                    elements: [
                                        ...
                                        {
                                            type: "emoji",
                                            name: `${EMOJIS[this.formatChangeData(numberOfUsers.data?.value || 0, numberOfUsers.data?.previous || 0)[1]]}`,
                                        },
                                        {
                                            type: "text",
                                            text: ` ${this.formatChangeData(numberOfUsers.data?.value || 0, numberOfUsers.data?.previous || 0)[0]})`,
                                        },
                                    ],
                                },
                                {
                                    ...
                                    elements: [
                                        ...
                                        {
                                            type: "emoji",
                                            name: `${EMOJIS[this.formatChangeData(numberOfThreats.data?.value || 0, numberOfThreats.data?.previous || 0)[1]]}`,
                                        },
                                        {
                                            type: "text",
                                            text: ` ${this.formatChangeData(numberOfThreats.data?.value || 0, numberOfThreats.data?.previous || 0)[0]})`,
                                        },
                                    ],
                                },
                                {
                                    ...
                                    elements: [
                                        ...
                                        {
                                            type: "emoji",
                                            name: `${EMOJIS[this.formatChangeData(numberOfSessions.data?.value || 0, numberOfSessions.data?.previous || 0)[1]]}`,
                                        },
                                        {
                                            type: "text",
                                            text: ` ${this.formatChangeData(numberOfSessions.data?.value || 0, numberOfSessions.data?.previous || 0)[0]})`,
                                        },
                                    ],
                                },
                                {
                                    ...
                                    elements: [
                                        ...
                                        {
                                            type: "emoji",
                                            name: `${EMOJIS[this.formatChangeData(avgCostOfSessions.data?.value || 0, avgCostOfSessions.data?.previous || 0)[1]]}`,
                                        },
                                        {
                                            type: "text",
                                            text: ` ${this.formatChangeData(avgCostOfSessions.data?.value || 0, avgCostOfSessions.data?.previous || 0)[0]})`,
                                        },
                                    ],
                                },
                            ],
                          ...
                          <td align="center" style="padding:30px;" class="content">
                            <table align="center" role="presentation" border="0" cellpadding="0" cellspacing="0" width="600" style="border-collapse:collapse;max-width:600px;width:100%;background-color:#fffffe;">
                              <tr style="vertical-align:middle;" valign="middle">
                                <td class="content">
                                  <ul>
                                    <li>Cost: $${cost.data?.value.toFixed(2)} (${this.formatChangeData(cost.data?.value || 0, cost.data?.previous || 0)[1]} by ${this.formatChangeData(cost.data?.value || 0, cost.data?.previous || 0)[0]})</li>
                                    <li>Requests: ${numberOfRequests.data?.value.toFixed(2)} (${this.formatChangeData(numberOfRequests.data?.value || 0, numberOfRequests.data?.previous || 0)[1]} by ${this.formatChangeData(numberOfRequests.data?.value || 0, numberOfRequests.data?.previous || 0)[0]})</li>
                                    <li>Error Rate: ${errorRate.data?.value.toFixed(2)} (${this.formatChangeData(errorRate.data?.value || 0, errorRate.data?.previous || 0, true)[1]} by ${this.formatChangeData(errorRate.data?.value || 0, errorRate.data?.previous || 0)[0]})</li>
                                    <li>Users: ${numberOfUsers.data?.value.toFixed(2)} (${this.formatChangeData(numberOfUsers.data?.value || 0, numberOfUsers.data?.previous || 0)[1]} by ${this.formatChangeData(numberOfUsers.data?.value || 0, numberOfUsers.data?.previous || 0)[0]})</li>
                                    <li>Threats: ${numberOfThreats.data?.value.toFixed(2)} (${this.formatChangeData(numberOfThreats.data?.value || 0, numberOfThreats.data?.previous || 0)[1]} by ${this.formatChangeData(numberOfThreats.data?.value || 0, numberOfThreats.data?.previous || 0)[0]})</li>
    <li>Sessions: ${numberOfSessions.data?.value.toFixed(2)} (${this.formatChangeData(numberOfSessions.data?.value || 0, numberOfSessions.data?.previous || 0)[1]} by ${this.formatChangeData(numberOfSessions.data?.value || 0, numberOfSessions.data?.previous || 0)[0]})</li>
    <li>Avg Cost of Sessions: $${avgCostOfSessions.data?.value.toFixed(2)} (${this.formatChangeData(avgCostOfSessions.data?.value || 0, avgCostOfSessions.data?.previous || 0)[1]} by ${this.formatChangeData(avgCostOfSessions.data?.value || 0, avgCostOfSessions.data?.previous || 0)[0]})</li>
      ...
      - Cost: $${cost.data?.value.toFixed(2)} (${this.formatChangeData(cost.data?.value || 0, cost.data?.previous || 0)[1]} by ${this.formatChangeData(cost.data?.value || 0, cost.data?.previous || 0)[0]})</li>
    - Requests: ${numberOfRequests.data?.value.toFixed(2)} (${this.formatChangeData(numberOfRequests.data?.value || 0, numberOfRequests.data?.previous || 0)[1]} by ${this.formatChangeData(numberOfRequests.data?.value || 0, numberOfRequests.data?.previous || 0)[0]})</li>
    - Error Rate: ${errorRate.data?.value.toFixed(2)} (${this.formatChangeData(errorRate.data?.value || 0, errorRate.data?.previous || 0, true)[1]} by ${this.formatChangeData(errorRate.data?.value || 0, errorRate.data?.previous || 0)[0]})</li>
    - Users: ${numberOfUsers.data?.value.toFixed(2)} (${this.formatChangeData(numberOfUsers.data?.value || 0, numberOfUsers.data?.previous || 0)[1]} by ${this.formatChangeData(numberOfUsers.data?.value || 0, numberOfUsers.data?.previous || 0)[0]})
    - Threats: ${numberOfThreats.data?.value.toFixed(2)} (${this.formatChangeData(numberOfThreats.data?.value || 0, numberOfThreats.data?.previous || 0)[1]} by ${this.formatChangeData(numberOfThreats.data?.value || 0, numberOfThreats.data?.previous || 0)[0]})
    - Sessions: ${numberOfSessions.data?.value.toFixed(2)} (${this.formatChangeData(numberOfSessions.data?.value || 0, numberOfSessions.data?.previous || 0)[1]} by ${this.formatChangeData(numberOfSessions.data?.value || 0, numberOfSessions.data?.previous || 0)[0]})
    - Avg Cost of Sessions: $${avgCostOfSessions.data?.value.toFixed(2)} (${this.formatChangeData(avgCostOfSessions.data?.value || 0, avgCostOfSessions.data?.previous || 0)[1]} by ${this.formatChangeData(avgCostOfSessions.data?.value || 0, avgCostOfSessions.data?.previous || 0)[0]})`;
        return { subject, text, html, slack_json };
    }
    formatChangeData(current, previous, inPercentage = false) {
        const change = previous === 0 || inPercentage
