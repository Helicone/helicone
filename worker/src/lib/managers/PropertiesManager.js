...
export async function handleLoggingEndpoint(request, env) {
    const body = await request.getJson();
    const heliconeId = body["helicone-id"];
    const propTag = "helicone-property-";
    const heliconeHeaders = Object.fromEntries([...request.getHeaders().entries()]
...
export async function updateRequestProperties(id, properties, env) {
