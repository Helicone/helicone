...
export class AlertManager {
    ...
    async checkAlerts() {
        const { data: allAlerts, error: allAlertsErr } = await this.alertStore.getAlerts();
        if (allAlertsErr) {
        ...
        const alertStatePromises = allAlerts.map(async (alert) => {
            const { data: alertState, error: alertStateErr } = await this.getAlertState(alert);
            if (alertStateErr || !alertState) {
                console.error(`Error retrieving alert state for alert ID ${alert.id}: ${alertStateErr}`);
                return;
            }
        ...
        const { error: updatedStateErr } = await this.handleAlertStateUpdates(triggeredAlerts, resolvedAlerts, now);
        if (updatedStateErr) {
            return err(`Error updating alert states: ${updatedStateErr}`);
        }
        return ok(null);
    }
    async handleAlertStateUpdates(triggeredAlerts, resolvedAlerts, now) {
        if (triggeredAlerts && triggeredAlerts.length > 0) {
            const { error: triggerAlertsErr } = await this.updateTriggeredAlerts(triggeredAlerts, now);
            if (triggerAlertsErr) {
                console.error(`Failed to trigger alerts: ${triggerAlertsErr}`);
            }
            const { error: sendEmailErr } = await this.sendAlertEmails(triggeredAlerts);
            if (sendEmailErr) {
                console.error(`Failed to send alert emails: ${sendEmailErr}`);
            }
            const { error: sendSlackErr } = await this.sendAlertSlacks(triggeredAlerts);
            if (sendSlackErr) {
                console.error(`Failed to send alert slacks: ${sendSlackErr}`);
            }
        }
        if (resolvedAlerts.length > 0) {
            const { error: resolvedAlertsErr } = await this.updateResolvedAlerts(resolvedAlerts, now);
            if (resolvedAlertsErr) {
                console.error(`Failed to resolve alerts: ${resolvedAlertsErr}`);
            }
            const { error: sendEmailErr } = await this.sendAlertEmails(resolvedAlerts);
            if (sendEmailErr) {
                console.error(`Failed to send resolve emails: ${sendEmailErr}`);
            }
            const { error: sendSlackErr } = await this.sendAlertSlacks(resolvedAlerts);
            if (sendSlackErr) {
    ...
    async updateResolvedAlerts(resolvedAlerts, now) {
        const alertIds = resolvedAlerts.map((alert) => alert.alert.id);
        const { error: alertUpdateErr } = await this.alertStore.updateAlertStatuses("resolved", alertIds);
        if (alertUpdateErr) {
            return err(`Error updating resolved alerts: ${JSON.stringify(alertUpdateErr)}`);
        }
        const { error: alertHistUpdateErr } = await this.alertStore.updateAlertHistoryStatuses("resolved", alertIds, now);
        if (alertHistUpdateErr) {
            return err(`Error updating alert history: ${JSON.stringify(alertHistUpdateErr)}`);
        }
        return ok(null);
    }
    async updateTriggeredAlerts(triggeredAlerts, now) {
        // Insert into alert history, update alert table status.
        const { error: alertUpdateErr } = await this.alertStore.updateAlertStatuses("triggered", triggeredAlerts.map((alert) => alert.alert.id));
        if (alertUpdateErr) {
            return err(`Error updating triggered alerts: ${JSON.stringify(alertUpdateErr)}`);
        }
        const { error: alertHistoryUpdateErr } = await this.alertStore.insertAlertHistory(triggeredAlerts.map((triggeredAlert) => {
            return {
    ...
    async getAlertState(alert) {
        if (alert.metric === "response.status") {
            return await this.alertStore.getErrorRate(alert.org_id, alert.time_window);
        }
    ...
    async getAlertStateUpdate(alert, alertState, timestamp) {
        ...
        return await this.handleRateAboveThreshold(alert, triggerThreshold, alertState.requestCount, timestamp);
    }
    ...
    async handleRateAboveThreshold(alert, triggerThreshold, requestCount, timestamp) {
        if (alert.status === "resolved" &&
    ...
    async deleteCooldown(alertId) {
        await this.utilityKv.delete(this.getCooldownStartTimestamp(alertId));
    }
    async getCooldownStart(alertId) {
        const timestampString = await this.utilityKv.get(this.getCooldownStartTimestamp(alertId));
        return timestampString !== null ? parseInt(timestampString, 10) : null;
    }
    async setCooldownStart(alertId, timestamp) {
        await safePut({
            key: this.utilityKv,
    ...
    getCooldownStartTimestamp = (alertId) => `alert:${alertId}:cooldown_start_timestamp`;
    async sendAlertEmails(alertStatusUpdates) {
        const promises = alertStatusUpdates.map(async (alertStatusUpdate) => {
            if (alertStatusUpdate.alert.emails.length > 0) {
                const { error: emailResErr } = await this.sendAlertEmail(alertStatusUpdate);
                if (emailResErr) {
    ...
    async sendAlertEmail(alertStatusUpdate) {
        const { subject, text, html } = this.formatAlertNotification(alertStatusUpdate);
    ...
    async sendAlertSlacks(alertStatusUpdates) {
        const promises = alertStatusUpdates.map(async (alertStatusUpdate) => {
            if (alertStatusUpdate.alert.slack_channels.length > 0) {
                const { error: slackResErr } = await this.sendAlertSlack(alertStatusUpdate);
                if (slackResErr) {
    ...
    async sendAlertSlack(alertStatusUpdate) {
        ...
        for (const channel of alertStatusUpdate.alert.slack_channels) {
            const slackIntegration = alertStatusUpdate.alert.organization.integrations.filter((integration) => integration.integration_name === "slack")[0];
            if (!slackIntegration) {
                return err(`Slack integration not found for alert ${alertStatusUpdate.alert.name}`);
            }
            const settings = slackIntegration.settings;
            if (!settings.access_token) {
                return err(`Slack access token not found for alert ${alertStatusUpdate.alert.name}`);
            }
    ...
    formatAlertNotification(alertStatusUpdate) {
        const alert = alertStatusUpdate.alert;
        const status = alertStatusUpdate.status ?? "";
        const capitalizedStatus = status.charAt(0).toUpperCase() + status.slice(1);
        const formatTimestamp = (timestamp) => timestamp ? new Date(timestamp).toUTCString() : "N/A";
        const formatTimespan = (ms) => {
            const minutes = Math.floor(ms / 60000);
            const hours = Math.floor(minutes / 60);
            return hours > 0 ? `${hours} hour(s)` : `${minutes} minute(s)`;
        };
        const headerClass = alertStatusUpdate.status === "triggered" ? "Triggered" : "Resolved";
        const headerEmoji = alertStatusUpdate.status === "triggered"
            ? ":warning:"
            : ":white_check_mark:";
        const subject = `Alert ${capitalizedStatus}: ${alert.name}`;
        const alertTime = new Date(alertStatusUpdate.timestamp).toUTCString();
        const actionMessage = status === "triggered"
            ? "Please take the necessary action."
            : "No further action is required.";
        const text = `Alert '${alert.name}' has been ${capitalizedStatus}.\n\nDetails:\n- ${capitalizedStatus} At: ${alertTime ? formatTimestamp(alertTime) : "alertTime not found"}\n- Threshold: ${alert.threshold}%\n\n${actionMessage}`;
        const slack_json = {
            blocks: [
                {
                    type: "header",
                ...
                },
                {
                    ...
                    text: {
                        type: "mrkdwn",
                        text: (alert.metric === "response.status"
                            ? alertStatusUpdate.status === "triggered"
                                ? `Error rate has breached *${alert.threshold}%* in the last *${formatTimespan(alert.time_window)}*.`
                                : `Error rate is now back within *${alert.threshold}%*.`
                            : alertStatusUpdate.status === "triggered"
                                ? `Cost has exceeded *$${alert.threshold}* in the last *${formatTimespan(alert.time_window)}*.`
                            ...
                            <table align="center" role="presentation" border="0" cellpadding="0" cellspacing="0" width="600" style="border-collapse:collapse;max-width:600px;width:100%;">
                              <tr style="vertical-align:middle;" valign="middle">
                                <td class="mso-padding alert alert-warning alert-dark" align="center" bgcolor="#000000" valign="top">
                                  <p style="font-size:20px; mso-line-height-rule:exactly; line-height:28px; font-family:Arial, sans-serif; vertical-align:top; color:${headerClass === "Resolved"
            ? "#00FF00"
            : "#FF0000"}; font-weight:500; text-align:center; border-radius:3px 3px 0 0; background-color:#000000; margin:0; padding:20px; margin-top:0!important; margin-bottom:0!important;">
                                  Helicone Alert ${headerClass.charAt(0).toUpperCase() +
            headerClass.slice(1)}<br/>${alert.name}
                                  </p>    
                                </td>
                              </tr>
                            </table>
                              ...
                              <tr style="vertical-align:middle;" valign="middle">
                                <td class="content">
                                  <ul style="list-style-type: none; padding: 0; margin: 0; color:#000000; font-size:16px; line-height:24px; font-family:Arial, sans-serif;">
                                    ...
                                    <li><strong>Triggered At:</strong> ${alertTime
            ? formatTimestamp(alertTime)
            : "alertTime not found"}</li>
                                    <li><strong>Threshold:</strong> ${alert.metric === "cost"
            ? `$${alert.threshold}`
            : `${alert.threshold}%`}</li>
                                    <li><strong>Metric:</strong> ${alert.metric}</li>
                                    <li><strong>Time Window:</strong> ${formatTimespan(alert.time_window)}
                                    </li>
