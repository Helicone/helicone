...
export class APIKeysManager {
    ...
    async setAPIKeys() {
        ...
        if (apiKeys) {
            ...
            await Promise.all(apiKeys.map(async (key) => {
                ...
                await storeInCache(`api_keys_${key.api_key_hash}`, key.organization_id, this.env);
            }));
        }
    }
    async setAPIKey(apiKeyHash, organizationId, softDelete) {
    ...
    async getAPIKeyWithFetch(apiKeyHash) {
        ...
        if (!key) {
            ...
            await storeInCache(`api_keys_${apiKeyHash}`, key.organization_id, this.env);
