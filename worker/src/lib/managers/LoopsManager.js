...
async function getAllUser(supabaseServer) {
    const allUsers = [];
    ...
    allUsers.push(...contactUs.map((user) => ({
        msft: user.tag === "mfs",
...
async function getAllOnboardedOrgs(supabaseServer) {
...
export async function updateLoopUsers(env) {
    const supabaseServer = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY);
    ...
    const cachedUserEmails = JSON.parse((await env.UTILITY_KV.get("loop_user_emails_v10")) ?? "[]");
    // Create fast lookup maps
    const onboardedOwnersMap = new Map((onboardedOrgs || []).map((org) => [org.owner, true]));
    // Create a map of cached users for fast lookup
    const cachedUserEmailMap = new Map(cachedUserEmails.map((user) => [
        user.email,
        {
    ...
    console.log(`LoopsManager: Onboarding check enabled: ${checkingOnboardingEnabled} (current date: ${currentDate.toISOString()}, start date: ${ONBOARDING_CHECK_START_DATE.toISOString()})`);
    ...
    for (const [email, userData] of uniqueUsersMap.entries()) {
        ...
        if (usersToProcess.length >= MAX_USERS_PER_RUN) {
            console.log(`LoopsManager: Early exit - found ${MAX_USERS_PER_RUN} users to update, stopping processing`);
            break;
        }
        ...
        if (cachedUser) {
            ...
            if (shouldCheckOnboarding && hasOnboarded !== cachedUser.hasOnboarded) {
                console.log(`LoopsManager: User ${email} onboarding status changed: ${cachedUser.hasOnboarded} -> ${hasOnboarded}`);
                onboardingStatusChanged = true;
                needsUpdate = true;
            }
            else if (hasOnboarded !== cachedUser.hasOnboarded) {
                console.log(`LoopsManager: User ${email} onboarding status changed but check is disabled: ${cachedUser.hasOnboarded} -> ${hasOnboarded}`);
            }
            // Check if tags changed
            const userTagsSet = new Set(userData.tags);
            const tagsAdded = userData.tags.filter((t) => !cachedUser.tags.has(t));
            const tagsRemoved = Array.from(cachedUser.tags).filter((t) => !userTagsSet.has(t));
    ...
    for (let i = 0; i < loopsUpdates.length; i += batchSize) {
        ...
        console.log(`Processing batch ${Math.floor(i / batchSize) + 1} of ${Math.ceil(loopsUpdates.length / batchSize)}`);
