...
// uses Dat trunc
export async function checkLimitsSingle(costUSD, requestCount, timeGrain, organizationId, env) {
    if (!organizationId) {
  ...
  `, [organizationId]);
    ...
    return ok(`OK, within limits {cost: ${cost}/${costUSD}, count: ${count}/${requestCount}}}`);
}
const generateSubquery = (index) => {
...
export async function checkLimits(limits, env) {
    ...
    const { data: keyMappings, error } = await client.dbQuery(query, limits.flatMap((limit) => [
    ...
    const limitResults = (Object.values(keyMappings?.[0])?.[0] ?? []);
    ...
    const result = limits.every((limit) => {
        const limitResult = remappedResults.find((result) => result.limitId === limit.id);
        if (!limitResult) {
            return false;
        }
        if (limit.cost !== null) {
