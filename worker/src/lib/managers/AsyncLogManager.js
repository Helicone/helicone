...
import { validateAsyncLogModel } from "../models/AsyncLog";
...
export async function logAsync(requestWrapper, env, ctx, provider) {
    const asyncLogModel = await requestWrapper.getJson();
    ...
    const heliconeHeaders = new HeliconeHeaders(mergeHeaders(requestHeaders, requestWrapper.headers));
    let templateWithInputs;
    try {
    ...
    }, env.S3_ENABLED ?? "true", heliconeHeaders);
    if (logError !== null) {
