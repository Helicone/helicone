...
export class DBQueryTimer {
    ctx;
    dataDogConfig;
    constructor(ctx, dataDogConfig) {
        this.ctx = ctx;
        this.dataDogConfig = dataDogConfig;
    }
    async withTiming(promise, { queryName, percentLogging = 1 }) {
        if (!this.ctx) {
        ...
        if (this.dataDogConfig.enabled && randomNumber < percentLogging) {
            this.ctx.waitUntil(this.logDistributionMetric(timestamp, end - start, queryName));
        }
        return result;
    }
    async logDistributionMetric(timestamp, executionTimeMs, queryName) {
        try {
            const distribution = {
                series: [
                    {
            ...
            const response = await fetch(`${this.dataDogConfig.endpoint}/v1/distribution_points`, requestInit);
            return response;
        }
        catch (e) {
