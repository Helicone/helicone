...
function isGoogleAuthHeader(value) {
    if (typeof value !== "string") {
        return false;
    }
    return value.split(" ").some((part) => part.startsWith("ya29."));
}
export async function kvKeyFromRequest(request, freeIndex, cacheSeed) {
    ...
    return await hash((cacheSeed ?? "") +
        request.url +
        (await request.requestWrapper.getText()) +
        JSON.stringify([...headers.entries()]) +
        (freeIndex >= 1 ? freeIndex.toString() : ""));
}
async function trySaveToCache(options) {
    try {
        const { request, response, responseBody, cacheControl, settings, cacheKv, cacheSeed, } = options;
        const expirationTtl = cacheControl.includes("max-age=")
            ? parseInt(cacheControl.split("max-age=")[1])
            : 0;
        const { freeIndexes } = await getMaxCachedResponses(request, settings, cacheKv, cacheSeed);
...
async function saveToCacheBackoff(options) {
...
export async function saveToCache(options) {
    return await saveToCacheBackoff(options);
}
export async function getCachedResponse(request, settings, cacheKv, cacheSeed) {
    ...
    try {
        ...
        if (freeIndexes.length > 0) {
            ...
            cachedResponseHeaders.append("Helicone-Cache-Latency", randomCache.latency ? randomCache.latency.toString() : "0");
...
async function getMaxCachedResponses(request, { bucketSize: bucketSize }, cacheKv, cacheSeed) {
    const previouslyCachedReqs = await Promise.all(Array.from(Array(bucketSize).keys()).map(async (idx) => {
        const requestCache = await kvKeyFromRequest(request, idx, cacheSeed);
        return cacheKv.get(requestCache, { type: "json" });
    }));
    return {
        requests: previouslyCachedReqs.filter((r) => r !== null),
