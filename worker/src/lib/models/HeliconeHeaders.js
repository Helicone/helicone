...
export class HeliconeHeaders {
    ...
    getFallBacks() {
        ...
        return parsedFallBacks.map((fb) => {
            ...
            if (typeof fb.headers !== "object" &&
                fb.headers.entries.every(([key, value]) => typeof key === "string" && typeof value === "string")) {
                throw new Error("helicone-fallbacks headers must be an object");
            }
            if (!Array.isArray(fb.onCodes) &&
                fb.onCodes.every((x) => typeof x === "number" ||
                    (typeof x === "object" &&
    ...
    determineBearerKeyType(bearerKey) {
        try {
            const key = bearerKey.replace("Bearer ", "").trim();
            if (HELICONE_RATE_LIMITED_API_KEY_REGEX.some((pattern) => pattern.test(key))) {
                return "rate-limited";
            }
            return "standard";
        }
    ...
    getGatewayConfig() {
        return {
            bodyMapping: this.headers.get("Helicone-Gateway-Body-Mapping") === "NO_MAPPING"
                ? "NO_MAPPING"
                : "OPENAI",
        };
    }
    getHeliconeHeaders() {
        const requestId = this.getValidUUID(this.headers.get("Helicone-Request-Id"));
        return {
            ...
            modelOverride: this.modelOverride ??
                ...
                null,
            promptSecurityEnabled: (this.headers.get("Helicone-LLM-Security-Enabled") ??
                this.headers.get("Helicone-Prompt-Security-Enabled") ??
                "").toLowerCase() === "true",
            promptSecurityAdvanced: this.headers.get("Helicone-LLM-Security-Advanced") ?? null,
            moderationsEnabled: this.headers.get("Helicone-Moderations-Enabled") == "true"
                ? true
                : false,
            ...
            webhookEnabled: this.headers.get("Helicone-Webhook-Enabled") == "true" ? true : false,
            experimentHeaders: {
            ...
            heliconeManualAccessKey: this.headers.get("Helicone-Manual-Access-Key") ?? null,
        };
    }
    getRetryHeaders() {
    ...
    getHeliconeProperties(heliconeHeaders) {
        const propTag = "helicone-property-";
        const heliconePropertyHeaders = Object.fromEntries([...this.headers.entries()]
            .filter(([key]) => key.toLowerCase().startsWith(propTag.toLowerCase()) &&
