...
async function getSegmentKeyValue(properties, userId, segment) {
...
function binarySearchFirstRelevantIndex(timestamps, now, timeWindowMillis) {
    let left = 0;
...
export async function checkRateLimit(props) {
    const { heliconeProperties, userId, rateLimitOptions, organizationId, rateLimitKV, } = props;
    const { time_window, segment, quota } = rateLimitOptions;
    const segmentKeyValue = await getSegmentKeyValue(heliconeProperties, userId, segment);
    ...
    const firstRelevantIndex = binarySearchFirstRelevantIndex(timestamps.map((x) => x.timestamp), now, timeWindowMillis);
    ...
    const reset = Math.ceil((timestamps[firstRelevantIndex].timestamp + timeWindowMillis - now) / 1000);
    if (currentQuota >= quota) {
        return { status: "rate_limited", limit: quota, remaining, reset };
    }
    return { status: "ok", limit: quota, remaining };
}
export async function updateRateLimitCounter(props) {
    const { heliconeProperties, userId, rateLimitOptions, organizationId, rateLimitKV, } = props;
