...
export function recursivelyConsolidateAnthropic(body, delta) {
...
export function recursivelyConsolidateAnthropicListForClaude(delta) {
...
export function getModel(requestBody) {
    try {
...
export async function anthropicAIStream(result, tokenCounter, requestBody) {
    const lines = result
        .split("\n")
        .filter((line) => line !== "")
        .map((line) => {
    ...
    try {
        if (getModel(requestBody ?? "{}").includes("claude-3") ||
            ...
            const promptTokens = await tokenCounter(JSON.parse(requestBody ?? "{}")?.prompt ?? "");
            return ok({
