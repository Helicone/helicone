...
import { err, ok } from "../lib/util/results";
import { buildProviderUrl, getProviderConfig as getUnifiedProviderConfig, } from "@helicone-package/cost/unified/providers";
...
const generateHandler = async (req, requestWrapper, env, ctx) => {
    try {
        // 1. VALIDATE AUTH AND GET ORG DATA
        const { data: auth, error: authError } = await requestWrapper.auth();
        if (authError || !auth) {
            return createErrorResponse(authError || "Invalid authentication", "auth_error", 401);
        }
        const db = new DBWrapper(env, auth);
        const { data: orgData, error: orgError } = await db.getAuthParams();
        if (orgError || !orgData) {
            return createErrorResponse(orgError || "Failed to get organization data", "org_error", 401);
        }
        // 2. BUILD GENERATE PARAMETERS FROM REQUEST BODY AND VALIDATE WITH ZOD
        const rawBody = await requestWrapper.getJson();
        const paramsResult = generateParamsSchema.safeParse(rawBody);
        if (!paramsResult.success) {
            return createErrorResponse("Invalid parameters", "invalid_parameters", 400, {
        ...
        if (promptResult.error || !promptResult.data) {
            return createErrorResponse(promptResult.error || "Failed to get prompt", "prompt_not_found", 404, {
                promptId: parameters.promptId,
                version: parameters.version,
            });
        }
        // 4. GET PROVIDER CONFIG (TARGET URL, PROVIDER, MAPPER)
        const metadata = promptResult.data.metadata;
        const providerResult = getProviderConfig(metadata, requestWrapper.getHeaders(), promptResult.data.helicone_template);
        if (providerResult.error || !providerResult.data) {
            return createErrorResponse(providerResult.error || "Failed to get provider info", "provider_error", 400, { metadata });
        }
        const { provider, mapper, targetUrl, authHeaderConfig, defaultHeaders } = providerResult.data;
        ...
        if (!providerApiKey) {
            return createErrorResponse(`Missing ${provider}_API_KEY in headers`, "missing_api_key", 400, { provider });
        }
        // b. Set basic headers
        const forwardHeaders = new Headers();
        forwardHeaders.set("Content-Type", "application/json");
        forwardHeaders.set("Helicone-Auth", `${requestWrapper.getHeaders().get("Helicone-Auth")}`);
        // Use the provider-specific auth header configuration
        forwardHeaders.set(authHeaderConfig.headerName, authHeaderConfig.valuePrefix
        ...
        if (parameters.properties?.sessionId) {
            forwardHeaders.set("Helicone-Session-Id", parameters.properties.sessionId);
        }
        if (parameters.properties?.cache) {
            forwardHeaders.set("Helicone-Cache", parameters.properties.cache.toString());
        }
        ...
        const { data: forwardRequestWrapper } = await RequestWrapper.create(newRequest, env);
        if (!forwardRequestWrapper) {
            return createErrorResponse("Failed to create request wrapper: ", "request_creation_failed", 500);
        }
        ...
        return await gatewayForwarder({
            ...
            setBaseURLOverride: forwardRequestWrapper.setBaseURLOverride.bind(forwardRequestWrapper),
        }, forwardRequestWrapper, env, ctx);
    }
    catch (e) {
        console.error("Error in generate route:", e);
        return createErrorResponse(e instanceof Error ? e.message : "An unexpected error occurred", "internal_server_error", 500, { stack: e instanceof Error ? e.stack : undefined });
    }
};
export const getGenerateRouter = (router) => {
 ...
 */
function getProviderConfig(metadata, headers, template) {
    // Validate provider exists
    const providerName = metadata.provider?.toUpperCase();
    if (!providerName) {
        return err("Provider configuration error: No provider specified in prompt metadata");
    }
    ...
    try {
        ...
        if (!config) {
            return err(`Provider configuration error: Unsupported provider ${providerName}`);
        }
 ...
 */
async function getPromptVersion({ supabaseClient, orgId, promptId, version = "production", }) {
    ...
    if (version === "production") {
        ...
        query = query.filter("prompts_versions.metadata->>isProduction", "eq", true);
    }
    else {
    ...
    if (!data || data.length === 0) {
        const versionText = version === "production" ? "production version" : `version ${version}`;
        return err(`Prompt not found: No ${versionText} of prompt "${promptId}" exists in your organization`);
    }
    // Extract the prompt version from the nested result
    const promptVersion = data[0].prompts_versions[0];
    if (!promptVersion) {
        return err(`Prompt version not found: The prompt "${promptId}" exists but has no valid versions`);
    }
 ...
 */
function createErrorResponse(message, code, status, details) {
 ...
 */
function addChatMessagesToTemplate(template, chatMessages) {
    ...
    if (chatMessages.length > 0) {
        ...
        if (userMessageIndex !== -1 && chatMessages.length > 0) {
            ...
            const insertPosition = systemMessageIndex !== -1 ? systemMessageIndex + 1 : 0;
        ...
        if (chatMessages.length > 2) {
            let nextRole = "user"; // Start with user since we just added/replaced an assistant message
