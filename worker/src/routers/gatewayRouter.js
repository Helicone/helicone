...
import { approvedDomains, providers, } from "@helicone-package/cost/providers/mappings";
...
async function rateLimitUnapprovedDomains(url, rateLimitKV) {
...
async function getProvider(targetBaseUrl, setBaseURLOverride, rateLimitKV, headers) {
    if (!targetBaseUrl) {
        return err(new Response("Missing target base url", {
            status: 400,
        }));
    }
    if (!validateURL(targetBaseUrl)) {
        err(new Response(`Invalid target base url "${targetBaseUrl}"`, {
            status: 400,
        }));
    }
    const targetBaseUrlHost = new URL(targetBaseUrl).origin;
    if (targetBaseUrlHost !== targetBaseUrl) {
        err(new Response(`Target base url "${targetBaseUrl}" must not contain a path, got "${targetBaseUrlHost}"`, {
            status: 400,
        }));
    }
    const { rateLimited } = await rateLimitUnapprovedDomains(targetBaseUrlHost, rateLimitKV);
    if (rateLimited) {
        err(new Response(`Rate limited unapproved domain! To get your target-url on the list of approved domains to surpase the rate limit please reach out to us at engineering@helicone.ai`, {
            status: 429,
        }));
    }
...
function getProviderFromTargetUrl(targetBaseUrl) {
    if (!targetBaseUrl) {
        return "CUSTOM";
    }
    const targetBaseUrlLowerCase = targetBaseUrl.toLowerCase();
    const provider = providers.find((provider) => provider.pattern.test(targetBaseUrlLowerCase));
    if (provider) {
        return provider.provider;
    }
    return targetBaseUrlLowerCase;
}
const gatewayForwarder = async (targetProps, requestWrapper, env, ctx) => {
    const { data: provderResults, error } = await getProvider(targetProps.targetBaseUrl, targetProps.setBaseURLOverride, env.RATE_LIMIT_KV, requestWrapper.headers);
    if (error) {
        return error;
    }
    return await proxyForwarder(requestWrapper, env, ctx, provderResults.provider);
};
const fallBack = async (requestWrapper, forwarder) => {
    ...
    for (const [i, fallback] of enumerate(fallbacks)) {
        const { "target-url": targetBaseUrl, headers, onCodes, bodyKeyOverride, } = fallback;
...
export const getGatewayAPIRouter = (router) => {
    // proxy forwarder only
    router.all("*", async (_, requestWrapper, env, ctx) => {
        function forwarder(targetBaseUrl) {
