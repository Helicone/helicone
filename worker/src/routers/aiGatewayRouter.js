...
import { getBody, authenticate, attemptModelRequestWithFallback, } from "../lib/util/aiGateway";
...
export const getAIGatewayRouter = (router) => {
    router.all("*", async (_, requestWrapper, env, ctx) => {
        ...
        const supabaseClient = isEU
            ? createClient(env.EU_SUPABASE_URL, env.EU_SUPABASE_SERVICE_ROLE_KEY)
            : createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY);
        const { orgId, rawAPIKey } = await authenticate(requestWrapper, env, new APIKeysStore(supabaseClient));
        if (!orgId || !rawAPIKey) {
        ...
        const result = await attemptModelRequestWithFallback({
            ...
            requestWrapper,
            forwarder,
            providerKeysManager: new ProviderKeysManager(new ProviderKeysStore(supabaseClient), env),
            promptManager: new PromptManager(new HeliconePromptManager({
                apiKey: rawAPIKey,
                baseUrl: env.VALHALLA_URL,
            }), new PromptStore(supabaseClient), env),
            orgId,
            parsedBody,
        });
        if (isErr(result)) {
