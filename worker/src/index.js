...
async function modifyEnvBasedOnPath(env, request) {
    ...
    if (request.isEU()) {
        env = {
            ...
            REQUEST_LOGS_QUEUE_URL_LOW_PRIORITY: env.EU_REQUEST_LOGS_QUEUE_URL_LOW_PRIORITY,
            S3_REGION: "eu-west-1",
    ...
    if ((host.includes("hconeai") || host.includes("helicone.ai")) &&
        hostParts.length >= 3) {
        ...
        if (hostParts[0].includes("ai-gateway")) {
            ...
            const forwardToHost = "bedrock-runtime." +
                url.pathname.split("/v1/")[1].split("/")[0] +
                ".amazonaws.com";
            const forwardToUrl = "https://" +
                forwardToHost +
                "/" +
...
export default {
    async fetch(request, env, ctx) {
        try {
            ...
            const router = buildRouter(env.WORKER_TYPE, request.url.includes("browser"));
            return router
    ...
    async scheduled(controller, env, _ctx) {
        ...
        if (controller.cron === "0 10 * * mon") {
            const reportManagerUS = new ReportManager(new ReportStore(supabaseClientUS, new ClickhouseClientWrapper({
            ...
            const reportManagerEU = new ReportManager(new ReportStore(supabaseClientEU, new ClickhouseClientWrapper({
                CLICKHOUSE_HOST: env.EU_CLICKHOUSE_HOST,
                CLICKHOUSE_USER: env.EU_CLICKHOUSE_USER,
                CLICKHOUSE_PASSWORD: env.EU_CLICKHOUSE_PASSWORD,
            })), env);
        ...
        if (controller.cron === "* * * * *") {
            const alertManagerUS = new AlertManager(new AlertStore(supabaseClientUS, new ClickhouseClientWrapper({
                CLICKHOUSE_HOST: env.CLICKHOUSE_HOST,
                CLICKHOUSE_USER: env.CLICKHOUSE_USER,
                CLICKHOUSE_PASSWORD: env.CLICKHOUSE_PASSWORD,
            })), env);
            ...
            const alertManagerEU = new AlertManager(new AlertStore(supabaseClientEU, new ClickhouseClientWrapper({
                CLICKHOUSE_HOST: env.EU_CLICKHOUSE_HOST,
                CLICKHOUSE_USER: env.EU_CLICKHOUSE_USER,
                CLICKHOUSE_PASSWORD: env.EU_CLICKHOUSE_PASSWORD,
            })), env);
