Anthropic's **Model Context Protocol (MCP)** has been a game-changer for how developers integrate tools, APIs, and services directly into AI applications. 

MCP's plug-and-play architecture allows you to create AI applications that interact with external systems seamlessly.

In this comprehensive guide, we'll cover how to use existing MCPs, build your own custom MCP server, and integrate Helicone for observability to monitor your AI usage.

## Table Of Contents

## How to Use a Prebuilt MCP

MCP's plug-and-play architecture lets you integrate pre-built tools into your AI environment with minimal configuration.

Take the Figma MCP—this open-source tool connects Figma files directly to **Cursor**, letting you query design specs and generate code through natural language.

Here's how to use it.

### Step 1: Get a Figma API Key

Sign up for a Figma account and create your personal API key following the [instructions here](https://help.figma.com/hc/en-us/articles/8085703771159-Manage-personal-access-tokens).

### Step 2: Add the MCP Server to Your Environment

Add the MCP server configuration to your AI development environment:
...
Now let's build our own MCP from scratch.

We'll build a Weather MCP that fetches current temperatures using the Open-Meteo API.
...
## Integrating Helicone for LLM Observability
...
**<span style={{color: '#0ea5e9'}}>Helicone</span>** provides robust observability for your LLM usage across providers like OpenAI, Claude, and Groq.
...
Alternatively, run this code to clone the complete repository and start your custom MCP:
...
Visit your [Helicone dashboard](https://www.helicone.ai/) to see the data:

![Helicone Dashboard](/static/blog/building-first-mcp-for-developers/helicone-dashboard.png)

For detailed analysis of individual requests, check the Requests page:
