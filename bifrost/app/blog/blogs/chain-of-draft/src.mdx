...
**Chain-of-Draft (CoD)** prompting, introduced by researchers at Zoom Communications, provides a solution by encouraging LLMs to generate minimal yet informative reasoning steps—similar to how humans jot down concise notes rather than elaborate explanations.

Let's explore this fascinating new method and see how you can start benefitting from it today.
...
CoD takes inspiration from human problem-solving behavior where we capture only essential information in our notes. Unlike Chain of Thought (CoT) prompting that generates verbose step-by-step reasoning, CoD limits each reasoning step to the absolute minimum—typically 5 words or less. This approach preserves the reasoning pathway while drastically cutting computational overhead.

### Chain-of-Draft and Chain of Thought Process Comparison
...
On **GSM8k** (a math reasoning benchmark), CoD achieved **91%** accuracy compared to CoT's **95%**, but used only **20%** of the tokens and reduced latency by up to **76%**. For symbolic reasoning tasks like the coin flip problem, CoD matched CoT's perfect 100% accuracy while using only **14-32%** of the tokens.
...
CoD performs best in **few-shot** settings (where you seed with examples), with accuracy dropping significantly in **zero-shot** (no examples) scenarios. This is likely due to the absence of CoD-style reasoning patterns in training data which makes it harder for models to 'understand' the approach. 
...
By embracing the minimalist reasoning approach of CoD, you can achieve comparable accuracy to Chain of Thought while significantly reducing costs and latency. This makes advanced reasoning capabilities more accessible for production applications with strict budgets and performance requirements. 

**Go ahead and play around with CoD and be sure to use Helicone to measure the improvements!**
