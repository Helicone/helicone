...
const llmResp = await openai.chat.completions.create(
  ...
  },
  {
    headers: {
      ...
      "Helicone-Prompt-Id": lastToolUsed
        ? `${lastToolUsed}-response`
        : "financial-research-reasoning",
    },
  }
);
```
...
const tools = [
  {
    ...
    function: {
      name: "getStockData",
      description:
        "Get current price and other market information for a specific stock by ticker symbol",
      parameters: {
        type: "object",
        properties: {
          ticker: {
  ...
  },
  {
    ...
    function: {
      name: "getStockNews",
      description:
        "Get the latest news articles for a specific stock by ticker symbol",
      parameters: {
        type: "object",
        properties: {
          ticker: {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Search for detailed company information in the knowledge base",
      parameters: {
        type: "object",
        properties: {
          query: {
...
async function processQuery(userQuery, vectorStore) {
  ...
  while (true) {
    ...
    if (lastToolUsed && lastToolArgs) {
      ...
      currentSessionHeaders["Helicone-Session-Path"] = getToolSessionPath(
        lastToolUsed,
        lastToolArgs
      );
    }
    ...
    if (msg.tool_calls && msg.tool_calls.length > 0) {
      ...
      const toolResult = await callHelper(
        functionName,
        functionArgs,
        vectorStore
      );
...
async function main() {
  console.log(
    "Initializing Financial Research Assistant with Helicone Sessions..."
  );
  ...
  const askQuestion = () => {
    rl.question("\nWhat would you like to know? ", async (query) => {
      if (query.toLowerCase() === "exit") {
        console.log(
          "Thank you for using the Financial Research Assistant. Goodbye!"
        );
        rl.close();
        return;
      }
