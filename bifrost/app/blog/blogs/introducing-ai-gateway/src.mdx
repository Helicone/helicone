...
Built in Rust for performance, the AI Gateway lets you switch between 100+ AI models without rewriting integrations or managing multiple API keys. The AI Gateway handles failovers, rate limiting, caching, and observability.
...
const response = await openai.chat.completions.create({
  model: "anthropic/claude-3-sonnet"  // Route to Claude via OpenAI syntax
});
```

Start routing AI traffic like the above.
...
**"The AI space moves so fast, new models drop weekly and we were constantly rewriting integrations."** - Engineering lead at a 50-person AI startup

**"We spent an entire sprint building custom fallback logic, and now we have to constantly maintain it to ensure our product is always reliable."** - CTO of Greptile.com

**"We outgrew our routing solution which couldn't scale for our growth and had to build our own. Now we're shopping for alternatives again."** - CTO of a 100-person AI startup

Every AI company hits the same problems.
...
Every team was building their own AI gateway or settling for subpar solutions because no one had built the definitive one yet. But once someone builds it right - in Rust, self-hosted, designed for scale, by people who've built routers for years - it becomes the default.
...
## Why This Will Become the Default
...
We're applying the same principles to AI routing. Rust for performance that doesn't degrade. Self-hosting so you never depend on our uptime. Apache license so you never worry about our business model. Deploy anywhere because your infrastructure choices shouldn't limit your routing options.

The AI gateway that wins will be the one that's fastest to deploy, hardest to break, and impossible to outgrow. That's what we built.
...
The AI Gateway is part of our bigger mission: helping AI teams ship fast without breaking things. Reliable infrastructure that just works, so you can focus on what makes your product unique.
