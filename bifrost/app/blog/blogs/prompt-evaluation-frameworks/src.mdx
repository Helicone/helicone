...
## Why Prompt Evaluation Frameworks Matter
...
![Helicon Promp Eval Dashboard](/static/blog/prompt-evaluation-frameworks/helicone.webp)
...
1. **Prompt Experiments**: Enables iterative refinement by creating prompt variations, testing them against production data, and validating results with custom evaluators. Supports side-by-side comparisons to optimize performance.  

2. **Scores API**: Assigns quantitative scores to requests and experiments, allowing detailed evaluation and comparison of prompt effectiveness.  

3. **Prompt Management**: Tracks, versions, and maintains datasets of inputs/outputs for each prompt iteration. Ensures seamless rollback and collaboration-friendly workflows.  

4. **Sessions**: Groups multi-step LLM interactions for debugging and optimizing complex workflows.  

5. **User Feedback**: Captures user responses (positive/negative) to measure and refine prompt success.  
