...
1. **Fact-conflicting hallucinations**: When LLMs generate information that contradicts known facts. _(e.g. Claiming a triangle has four sides, or that strawberries have 2 'r's.)_

2. **Input-conflicting hallucinations**: When outputs diverge from what was specifically requested or contradict the input material. _(e.g. Including statistics in a summary that weren't in the original text.)_

3. **Context-conflicting hallucinations**: When LLMs produce self-contradictory responses, particularly notable in longer outputs. _(e.g. "Abraham Lincoln was born on February 12, 1809, in LaRue County, Kentucky" - conflicting with an earlier stated fact that he was born in Hardin County.)_
