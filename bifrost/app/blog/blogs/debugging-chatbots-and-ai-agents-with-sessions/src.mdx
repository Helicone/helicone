...
While developing a chatbot similar to the one described above, an issue was discovered where the LLM occasionally provided incorrect parameters to the flight search API, leading to incorrect bookings.

**How to Debug Using Helicone Sessions**

1. **Open the Sessions dashboard**
    
    To examine what's causing the error, visit the Sessions dashboard and examine the requests related to the action(s) where the error occured. 
...
2. **Observe the Request**
    ...
    Next, click on the request to inspect its details. In this case, it was observed that the LLM sometimes misinterprets capitalized words as airline abbreviations..
   
    For example, given the user input: 
    
    "I've just arrived at **JFK**, where can I go to **reLAX**?" 

    The LLM mistakenly assumes the user wants to book a flight from JFK (New York) to LAX (Los Angeles).

3. **Fix bug in Playground**

    To fix this, we click the **Playground** button above the request details to experiment with different prompt variations.

Through testing, it was observed that converting the entire string to lowercase before passing it to the LLM allowed it to correctly parse airport codes while ignoring unrelated capitalized words.
...
With this fix applied, the chatbot now accurately interprets user intent and returns the correct departure and arrival airports, leading to more reliable flight search results.
...
Users report that a health & fitness chatbot that works similarly to the one shown above is generating inconsistent dietary recommendations over multiple interactions. 

For example, a user who previously selected a high-protein diet for muscle gain might later receive a low-calorie meal plan designed for weight loss, creating confusion and dissatisfaction.
...
1. **Open the Sessions Dashboard**  
   ...
   Verifying that users' medical records are being correctly analyzed seems like a good first step.

2. **Analyze the Medical History Retrieval Step**  

   In the `medical-history` trace, check if the chatbot is correctly retrieving users' medical history data from the vector database by viewing the trace's details.  
   
   In this case, it was discovered that the LLM was failing to properly lookup relevant user data from the vector database, causing inconsistencies in retrieved dietary preferences.
