...
import { ChevronDown, Copy, Check, KeyRound, ArrowUpRight, Info } from "lucide-react";
...
import { StandardParameter } from "@helicone/cost/models/types";
...
export function ModelDetailPage({ initialModel }: ModelDetailPageProps) {
  ...
  const [currentLanguage, setCurrentLanguage] = useState<"typescript" | "python">("typescript");
            ...
            <div className="flex flex-col gap-3 mt-4">
              <div className="flex items-center gap-2 text-sm">
                ...
                {model.created && (
                  <span className="text-gray-500 dark:text-gray-400">
                    Released {new Date(model.created).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}
                  </span>
                )}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm">
                <div>
                  ...
                  <span className="font-semibold text-gray-900 dark:text-gray-100">{formatContext(model.contextLength)}</span>
                </div>
                <div>
                  <span className="text-gray-500 dark:text-gray-400">Max Output: </span>
                  <span className="font-semibold text-gray-900 dark:text-gray-100">{model.maxOutput ? formatContext(model.maxOutput) : "—"}</span>
                </div>
                <div>
                  <span className="text-gray-500 dark:text-gray-400">Input: </span>
                  <span className="font-semibold text-gray-900 dark:text-gray-100">{formatCost(basePricing.prompt)}<span className="text-xs text-gray-500 dark:text-gray-400">/1M</span></span>
                </div>
                <div>
                  <span className="text-gray-500 dark:text-gray-400">Output: </span>
                  <span className="font-semibold text-gray-900 dark:text-gray-100">{formatCost(basePricing.completion)}<span className="text-xs text-gray-500 dark:text-gray-400">/1M</span></span>
                </div>
              </div>
            </div>
                      ...
                      >
                                    ...
                                    <TooltipProvider>
                                      <Tooltip>
                                        ...
                                        <TooltipContent className="max-w-xs">
                                          <p>OpenRouter is used as a fallback when rate limits are reached to ensure uninterrupted service. Credits shown are worst-case escrow; actual charges match OpenRouter's pricing.</p>
                                        </TooltipContent>
                                      </Tooltip>
                                    </TooltipProvider>
                                  )}
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 text-sm">
                          <div>
                            ...
                            <span className="font-mono text-gray-900 dark:text-gray-100">{formatContext(model.contextLength)}</span>
                          </div>
                          <div>
                            <span className="text-xs text-gray-500 dark:text-gray-400 block">Max Output</span>
                            <span className="font-mono text-gray-900 dark:text-gray-100">{model.maxOutput ? formatContext(model.maxOutput) : "—"}</span>
                          </div>
                          <div>
                            ...
                            <span className="font-mono text-gray-900 dark:text-gray-100">
                              {formatCost(pricing.prompt)}<span className="text-xs text-gray-500 dark:text-gray-400">/1M</span>
                              {endpoint.provider === "OpenRouter" && (
                                <span className="text-xs text-amber-600 dark:text-amber-400 ml-1">*</span>
                              )}
                            </span>
                          </div>
                          <div>
                            ...
                            <span className="font-mono text-gray-900 dark:text-gray-100">
                              {formatCost(pricing.completion)}<span className="text-xs text-gray-500 dark:text-gray-400">/1M</span>
                              {endpoint.provider === "OpenRouter" && (
                                <span className="text-xs text-amber-600 dark:text-amber-400 ml-1">*</span>
                              )}
                            </span>
                          ...
                          <div>
                            ...
                            <span className="font-mono text-gray-900 dark:text-gray-100">{pricing.cacheRead ? `${formatCost(pricing.cacheRead)}/1M` : "—"}</span>
                          </div>
                          <div>
                            <span className="text-xs text-gray-500 dark:text-gray-400 block">Cache Write</span>
                            <span className="font-mono text-gray-900 dark:text-gray-100">{pricing.cacheWrite ? `${formatCost(pricing.cacheWrite)}/1M` : "—"}</span>
                          </div>
                          {(pricing.image || pricing.audio) && (
                            <div>
                              ...
                              <span className="font-mono text-gray-900 dark:text-gray-100">
                                {pricing.image ? formatCost(pricing.image) : pricing.audio ? formatCost(pricing.audio) : "—"}
                              </span>
                            </div>
                          )}
                        </div>
                        ...
                        {isExpanded && (
                          <div className="px-4 pb-4 space-y-4">
                              ...
                              <div className="p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded text-xs">
                                <div className="flex items-start gap-2">
                                  ...
                                  <div className="text-amber-800 dark:text-amber-200">
                                    ...
                                    <p>OpenRouter routes to multiple providers with different costs. Prices shown are the maximum (worst-case) for credit escrow. You'll be charged actual costs, which are typically lower.</p>
                                  </div>
                                </div>
                              </div>
                            )}
                            ...
                            {hasTiers && pricingArray && pricingArray.length > 1 && (
                              <div>
                                <p className="text-xs font-medium text-gray-600 dark:text-gray-400 mb-2">Tiered Pricing</p>
                                <div className="space-y-1">
                                  {pricingArray.map((tier, tierIndex) => (
                                    <div key={tierIndex} className="flex items-center gap-4 text-xs">
                                      <span className="text-gray-500 dark:text-gray-400">
                                        {tierIndex === 0 ? "First" : `After ${tier.threshold ? formatContext(tier.threshold) : '0'}`}:
                                      </span>
                            ...
                            {model.supportedParameters && model.supportedParameters.length > 0 && (
                              <div>
                                <p className="text-xs font-medium text-gray-600 dark:text-gray-400 mb-2">Supported Parameters</p>
                                <div className="flex flex-wrap gap-1">
                                  {model.supportedParameters.includes("tools" as StandardParameter) && (
                                    <span className="text-xs px-2 py-0.5 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300">Tools</span>
                                  )}
                                  {model.supportedParameters.includes("response_format" as StandardParameter) && (
                                    <span className="text-xs px-2 py-0.5 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300">JSON Mode</span>
                                  )}
                                  {model.supportedParameters.includes("structured_outputs" as StandardParameter) && (
                                    <span className="text-xs px-2 py-0.5 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300">Structured Output</span>
                                  )}
                                  {model.supportedParameters.includes("stream" as StandardParameter) && (
                                    <span className="text-xs px-2 py-0.5 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300">Streaming</span>
                                  )}
                                  {model.supportedParameters.includes("max_tokens" as StandardParameter) && (
                                    <span className="text-xs px-2 py-0.5 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300">Max Tokens</span>
                                  )}
                                  {model.supportedParameters.includes("temperature" as StandardParameter) && (
                                    <span className="text-xs px-2 py-0.5 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300">Temperature</span>
                                  )}
                                  {model.supportedParameters.includes("top_p" as StandardParameter) && (
                                    <span className="text-xs px-2 py-0.5 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300">Top P</span>
                                  )}
                                  {model.supportedParameters.includes("logprobs" as StandardParameter) && (
                                    <span className="text-xs px-2 py-0.5 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300">Log Probs</span>
                                  )}
                                </div>
                              </div>
                            )}
    ...
    messages=[{"role": "user", "content": "Hello!"}],
              ...
              <div className="mt-4">
                <Link
                  ...
                  rel="noopener"
                >
                  <Button variant="ghost" size="sm" className="gap-1 px-0 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100">
                    View documentation
