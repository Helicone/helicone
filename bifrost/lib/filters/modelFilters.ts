...
export const filterByProviders = (
  ...
  return models.filter((model) =>
    model.endpoints.some((ep) =>
      Array.from(providers).some(
        (p) => ep.provider.toLowerCase() === p.toLowerCase(),
      ),
    ),
  );
};
...
export const filterByAuthors = (
  ...
  return models.filter((model) =>
    Array.from(authors).some(
      (a) => model.author.toLowerCase() === a.toLowerCase(),
    ),
  );
};
...
// Context size filter
export const filterByContextSize = (
  models: Model[],
  minContextSize: number,
): Model[] => {
...
export const filterByInputModalities = (
  models: Model[],
  inputModalities: Set<InputModality>,
): Model[] => {
  ...
  return models.filter((model) =>
    Array.from(inputModalities).some((m) => model.inputModalities.includes(m)),
  );
};

// Output modality filter
export const filterByOutputModalities = (
  models: Model[],
  outputModalities: Set<OutputModality>,
): Model[] => {
...
// Parameters filter
export const filterByParameters = (
  models: Model[],
  parameters: Set<StandardParameter>,
): Model[] => {
  ...
  return models.filter((model) =>
    Array.from(parameters).some((p) => model.supportedParameters.includes(p)),
  );
};
...
export const filterByCapabilities = (
  ...
  return models.filter((model) => {
    ...
    return Array.from(capabilities).some((c) =>
      modelCapabilities.has(c as ModelCapability),
    );
  });
};
...
export const filterByPtb = (models: Model[], showPtbOnly: boolean): Model[] => {
  ...
  return models.filter((model) => model.endpoints.some((ep) => ep.supportsPtb));
};
