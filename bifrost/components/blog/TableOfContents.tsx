  ...
  useEffect(() => {
    ...
    headingData.forEach((heading) => {
      ...
      // For all other headings, only include them if they're not in a FAQ section
        ...
        if (
          !effectiveExcludeHeadings.includes(heading.text) &&
          !matchesPattern
        ) {
          processedHeadings.push(heading);
        }
        ...
        <ul className="m-0 flex flex-col gap-1 pl-0">
          {headings.map((heading, index) => (
            <li
              key={heading.id}
              className={cn(
                "flex items-start pl-2 transition-colors",
                heading.level === 2 ? "ml-0" : "",
                heading.level === 3 ? "ml-8" : "",
                heading.level === 4 ? "ml-16" : ""
              )}
            >
              <span className="mr-3 mt-2.5 block h-1.5 w-1.5 min-w-1.5 rounded-full bg-slate-300 dark:bg-slate-600"></span>
              <Link
                href={`#${heading.id}`}
                className={cn(
                  "hover:text-primary font-normal transition-colors",
