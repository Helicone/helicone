        ...
        {PROVIDERS.map((provider, index) => (
          <li key={index} className="hover:bg-slate-100 hover:rounded-lg pl-3 pt-2 pr-1">
            ...
            >
              ...
              <span
                className={`text-sm font-semibold text-left pt-2 md:text-nowrap ${currentProvider === provider.name
          ...
          <ul className="flex items-center space-x-0 gap-3">
            {Object.keys(selectedProvider?.integrations || {}).map(
              (integration) => (
                <li
                  key={integration}
                  className={`cursor-pointer text-base font-medium px-6 py-2 border border-gray-500 rounded-xl ${currentIntegration === integration
