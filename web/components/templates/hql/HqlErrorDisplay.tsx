...
const ERROR_DISPLAY_MAP: Record<HqlErrorCode, Omit<ErrorDisplay, "description">> = {
  // SQL Validation Errors
  [HqlErrorCode.INVALID_STATEMENT]: {
    title: "Invalid SQL Statement",
    severity: "error",
    icon: XCircle,
    suggestions: ["Only SELECT statements are allowed", "Remove any INSERT, UPDATE, DELETE, or DDL statements"],
  },
  [HqlErrorCode.INVALID_TABLE]: {
    title: "Invalid Table",
    severity: "error",
    icon: XCircle,
    suggestions: ["Check the table name in your query", "Only authorized tables can be queried"],
  },
  [HqlErrorCode.SYNTAX_ERROR]: {
    title: "SQL Syntax Error",
    severity: "warning",
    icon: AlertTriangle,
    suggestions: ["Check your SQL syntax", "Verify all keywords are spelled correctly", "Ensure quotes and parentheses are balanced"],
  },
  ...
  [HqlErrorCode.QUERY_TIMEOUT]: {
    ...
    severity: "warning",
    icon: AlertTriangle,
    suggestions: ["Add more specific filters to reduce data", "Use LIMIT to restrict results", "Optimize your WHERE conditions"],
  },
  [HqlErrorCode.MEMORY_LIMIT_EXCEEDED]: {
    title: "Memory Limit Exceeded",
    severity: "warning",
    icon: AlertTriangle,
    suggestions: ["Reduce the number of columns selected", "Add date range filters", "Use aggregations instead of raw data"],
  },
  [HqlErrorCode.ROW_LIMIT_EXCEEDED]: {
    title: "Too Many Rows",
    severity: "warning",
    icon: AlertTriangle,
    suggestions: ["Add more WHERE conditions", "Use a smaller date range", "Apply LIMIT to your query"],
  },
  [HqlErrorCode.RESULT_LIMIT_EXCEEDED]: {
    title: "Result Set Too Large",
    severity: "warning",
    icon: AlertTriangle,
    suggestions: ["Add a LIMIT clause (max 10,000 rows)", "Export to CSV for larger datasets"],
  },
  ...
  [HqlErrorCode.NO_DATA_RETURNED]: {
    ...
    severity: "info",
    icon: AlertCircle,
    suggestions: ["Adjust your filters", "Check the date range", "Verify the table contains data"],
  },
...
export function HqlErrorDisplay({ error, className }: HqlErrorDisplayProps) {
  ...
  const { title, description, severity, icon: Icon, suggestions } = getErrorDetails(error);
  
  return (
    <Alert 
      variant={severity === "error" ? "destructive" : severity === "info" ? "default" : "default"}
      className={className}
    >
