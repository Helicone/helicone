...
import { HeliconeErrorType } from "../../heliconeErrorType";
...
export type OpenAIResponseBody = ChatCompletion | HeliconeErrorType;
...
const getRequestMessages = (request: OpenAIRequestBody) => {
  return (
    request.messages
      // Handle tool_result
      ?.map((msg: any) => {
...
const getLLMSchemaResponse = (response: OpenAIResponseBody) => {
  if ("error" in response) {
    ...
    return {
      ...
      model: (response as ChatCompletion)?.model,
    };
  }
};

export const mapOpenAIRequest: MapperFn<
  OpenAIRequestBody,
  OpenAIResponseBody
