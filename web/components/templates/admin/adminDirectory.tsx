...
export function Directory({
  ...
  const { mutateAsync: handleSaveQueryAsync } = useMutation({
    mutationFn: async (savedQuery: {
      ...
      if (savedQuery.id) {
        const response = await $JAWN_API.PATCH(
        ...
        const response = await $JAWN_API.POST("/v1/admin/saved-query", {
          body: {
            name: savedQuery.name,
            sql: savedQuery.sql,
          },
...
function QueryList({
  ...
  // Admin-specific delete mutation
  ...
  // Admin-specific bulk delete mutation
  const bulkDeleteMutation = useMutation({
    mutationFn: async (queryIds: string[]) => {
      // Delete each query individually since we don't have bulk delete endpoint
      await Promise.all(
        queryIds.map((queryId) =>
          $JAWN_API.DELETE("/v1/admin/saved-query/{queryId}", {
            params: { path: { queryId } },
          }),
        ),
      );
