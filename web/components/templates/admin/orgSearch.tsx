...
import { Search, Loader2, Trash2, UserPlus, X } from "lucide-react";
...
const OrgSearch = () => {
  ...
  return (
    <div className="flex h-full w-full flex-col gap-4 p-6 overflow-hidden">
...
const FeatureFlagsSection = ({ orgId, orgName }: { orgId: string; orgName: string }) => {
  const queryClient = useQueryClient();
  const { setNotification } = useNotification();
  ...
  const { data: featureFlagsData, isLoading: isLoadingFlags } = useQuery({
    ...
    queryFn: async () => {
      const jawn = getJawnClient();
      const { data, error } = await jawn.POST("/v1/admin/feature-flags/query", {});
      if (error) throw error;
  ...
  const deleteFeatureFlagMutation = useMutation({
    ...
    onError: (error: any) => {
      setNotification(error.message || "Failed to remove feature flag", "error");
    },
  });
    ...
    <div className="flex flex-col gap-2">
      ...
      {isLoadingFlags ? (
        <div className="flex items-center gap-2 py-4">
          ...
          <Muted className="text-xs">Loading feature flags...</Muted>
        </div>
      ) : (
        <>
          <div className="flex flex-wrap gap-2 min-h-[2.5rem] rounded-lg border border-border bg-background p-3">
            {flags.length > 0 ? (
              flags.map((flag: string) => (
                ...
                >
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      if (confirm(`Remove feature flag "${flag}"?`)) {
                        deleteFeatureFlagMutation.mutate(flag);
                      }
                    }}
              ...
              <Muted className="text-xs">No feature flags</Muted>
            )}
          </div>
