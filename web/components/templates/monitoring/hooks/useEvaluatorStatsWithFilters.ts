...
import { DEFAULT_STATS } from "../../evals/hooks/useEvaluatorStats";
...
export const useEvaluatorStatsWithFilters = (
  evaluatorId: string,
  ...
  return useQuery({
    queryKey: ["evaluatorStats", evaluatorId, org?.currentOrg?.id, timeFilter],
    queryFn: async () => {
      if (!evaluatorId) {
