...
<Tabs>
  ...
  <Tab title="Without OpenTelemetry">
    **Stdout logging without OpenTelemetry**
    ...
    Best for:
    - Deployments that don't contain an OTel stack
    - Flexible troubleshooting
    - Ease of local development
  </Tab>

  <Tab title="Development">
    **Console log output + Distributed Tracing Enabled**
    ...
    Perfect for development and debugging the Gateway:
    - Pretty-printed logs to console
    - Full distributed tracing enabled via OTel
      with Loki, Prometheus, and Tempo running in
      docker compose
  </Tab>
...
<Tabs>
  <Tab title="Grafana Stack">
    **Complete observability with Grafana, Prometheus, Tempo, and Loki**
    ...
    Then you'll want to import the provided Grafana dashboard by
    import the JSON dashboard from `infrastructure/grafana/dashboards/helicone-router.json`.
    Then, sending requests to the AI Gateway will begin to populate metrics!
    ...
    **Configured services:**
    ...
    - **Redis** (port 6379) - Caching and rate limiting, if configured
    - **OpenTelemetry Collector** (port 4317) - Telemetry ingestion

    <Note>
      The default telemetry config works with our provided docker compose stack.
      For your own deployment, you may need to customize the 
      [`telemetry.otlp-endpoint`](/gateway/config#telemetry).
    </Note>
  </Tab>
...
<Steps>
  <Step title="Resource Configuration">
    **Configure telemetry for production scale**
    ...
    ```yaml
    telemetry:
      level: "info"  # Avoid trace in production
...
export AI_GATEWAY__TELEMETRY__LEVEL="info,ai_gateway=debug"
export AI_GATEWAY__TELEMETRY__SERVICE_NAME="conduit-staging"
export AI_GATEWAY__TELEMETRY__EXPORTER="otlp"
export AI_GATEWAY__TELEMETRY__OTLP_ENDPOINT="http://otel-collector:4317"
export AI_GATEWAY__TELEMETRY__PROPAGATE="true"
```

<Note>
  Environment variables use double underscores (`__`) to separate nested configuration keys and follow the `AI_GATEWAY_` prefix convention.
</Note>
