...
describe("Perplexity Model Registry", () => {
  ...
  describe("Cost Calculations", () => {
    it("should calculate cost correctly for sonar", () => {
      ...
      const breakdown = calculateModelCostBreakdown({
        ...
        pricing: endpoint.pricing[0],
      });

      expect(breakdown.inputCost).toBe(1.0); // $1.00 for 1M input tokens
      expect(breakdown.outputCost).toBe(1.0); // $1.00 for 1M output tokens
      expect(breakdown.requestCost).toBe(5.0); // $5.00 for 1K requests
      expect(breakdown.totalCost).toBe(7.0); // Total: $7.00
    });
    ...
    it("should calculate cost correctly for sonar-pro", () => {
      ...
      expect(breakdown.inputCost).toBe(3.0); // $3.00 for 1M input tokens
      expect(breakdown.outputCost).toBe(15.0); // $15.00 for 1M output tokens
      expect(breakdown.requestCost).toBe(6.0); // $6.00 for 1K requests
      expect(breakdown.totalCost).toBe(24.0); // Total: $24.00
    });
    ...
    it("should calculate cost with web search", () => {
      ...
      expect(breakdown.inputCost).toBe(0.1); // $0.10 for 100K input tokens
      expect(breakdown.outputCost).toBe(0.1); // $0.10 for 100K output tokens
      expect(breakdown.requestCost).toBe(0.5); // $0.50 for 100 requests
      expect(breakdown.webSearchCost).toBe(0.5); // $0.50 for 100 searches (100 * 0.005)
      expect(breakdown.totalCost).toBe(1.2); // Total: $1.20
    });
  ...
  describe("Model Consistency", () => {
    ...
    it("should have unique names for all models", () => {
      const names = Object.values(perplexityModels).map((m) => m.name);
