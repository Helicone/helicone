...
describe("FilterAST", () => {
  ...
  describe("Filter Simplification", () => {
    test('should return condition and "all" expressions as-is', () => {
      ...
      expect(FilterAST.simplifyFilter(conditionFilter)).toEqual(
        conditionFilter,
      );
    });
  ...
  describe("NOT Operation", () => {
    test("should create NOT condition for simple operators", () => {
      ...
      expect(
        (notCondition.expressions[0] as ConditionExpression).operator,
      ).toBe("neq");
    });
    ...
    test("should handle comparison operators correctly", () => {
      ...
      expect((notGt.expressions[0] as ConditionExpression).operator).toBe(
        "lte",
      );
    });
