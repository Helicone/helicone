import { FilterAST, FilterExpression } from "../../filters/filterExpressions";
import {
  buildFilter,
  buildFilterLeaf,
  FilterNotImplemented,
} from "../../filters/filters";
import { FilterLeaf, FilterBranch, FilterNode } from "../../filters/filterDefs";
import {
...
describe("Edge Cases and Error Conditions", () => {
  ...
  describe("Filter Building Edge Cases", () => {
    ...
    test("should handle very long argument arrays", () => {
      ...
      // Add many conditions
      for (let i = 0; i < 100; i++) {
        (manyConditions.request_response_rmt as any)[`field_${i}`] = {
          equals: `value_${i}`,
        };
      }
  ...
  describe("FilterNotImplemented Error Handling", () => {
    ...
    test("should be catchable as specific error type", () => {
      try {
        ...
        expect(error).toBeInstanceOf(Error);
        expect((error as FilterNotImplemented).name).toBe(
          "FilterNotImplemented",
        );
  ...
  describe("Memory and Performance Edge Cases", () => {
    test("should handle large filter expressions without memory issues", () => {
      let largeFilter: FilterExpression = FilterAST.condition(
        "status",
        "eq",
        200,
      );
      ...
      for (let i = 0; i < 100; i++) {
        // Reduced from 1000 to avoid excessive test time
  ...
  describe("Boundary Value Testing", () => {
    test("should handle minimum and maximum safe integers", () => {
      const minFilter = FilterAST.condition(
        "value",
        "eq",
        Number.MIN_SAFE_INTEGER,
      );
      const maxFilter = FilterAST.condition(
        "value",
        "eq",
