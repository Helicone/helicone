...
describe("Frontend Filter Definitions", () => {
  ...
  describe("DASHBOARD_PAGE_TABLE_FILTERS", () => {
    test("should have correct structure and length", () => {
      ...
      const [modelFilter, statusFilter, latencyFilter, userFilter] =
        DASHBOARD_PAGE_TABLE_FILTERS;
  ...
  describe("REQUEST_TABLE_FILTERS", () => {
    ...
    test("should have correct categories", () => {
      const requestFilters = REQUEST_TABLE_FILTERS.filter(
        (f) => f.category === "request",
      );
      const responseFilters = REQUEST_TABLE_FILTERS.filter(
        (f) => f.category === "response",
      );
      const feedbackFilters = REQUEST_TABLE_FILTERS.filter(
    ...
    test("should have vector operators for request and response body", () => {
      const requestFilter = REQUEST_TABLE_FILTERS.find(
        (f) => f.label === "Request",
      );
      const responseFilter = REQUEST_TABLE_FILTERS.find(
        (f) => f.label === "Response",
      );
    ...
    test("should have number operators for token fields", () => {
      ...
      tokenFields.forEach((fieldLabel) => {
        const filter = REQUEST_TABLE_FILTERS.find(
          (f) => f.label === fieldLabel,
        );
        expect(filter?.operators).toHaveLength(4); // number operators
        expect(filter?.operators.some((op) => op.value === "equals")).toBe(
          true,
        );
    ...
    test("should have boolean operators for feedback", () => {
      const feedbackFilter = REQUEST_TABLE_FILTERS.find(
        (f) => f.label === "Feedback",
      );
    ...
    test("should have status filter with suggestions", () => {
      const statusFilter = REQUEST_TABLE_FILTERS.find(
        (f) => f.label === "Status",
      );
    ...
    test("should have text operators for request-id with boolean-to-text mapping", () => {
      const requestIdFilter = REQUEST_TABLE_FILTERS.find(
        (f) => f.label === "Request-Id",
      );

      expect(requestIdFilter?.column).toBe("request_id");
      expect(requestIdFilter?.operators).toHaveLength(2); // equals and not-equals
      expect(
        requestIdFilter?.operators.some((op) => op.value === "equals"),
      ).toBe(true);
      expect(
        requestIdFilter?.operators.some((op) => op.value === "not-equals"),
      ).toBe(true);
      expect(requestIdFilter?.operators.every((op) => op.type === "text")).toBe(
        true,
      );
    });
  });
