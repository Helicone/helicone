...
def run_curl_command(query, host, port, user=None, password=None, migration_file=None):
    if not query:
        if user and password:
            curl_cmd = (
                f'cat "{migration_file}" | curl --user "{user}:{password}" '
                f'"{get_host(host)}:{port}/" --data-binary @-'
            )
        else:
            curl_cmd = (
                f'cat "{migration_file}" | curl "{get_host(host)}:{port}/" --data-binary @-'
            )
    else:
        if user and password:
            curl_cmd = (
                f'echo "{query}" | curl --user "{user}:{password}" '
                f'"{get_host(host)}:{port}/" --data-binary @-'
            )
        else:
            curl_cmd = (
                f'echo "{query}" | curl "{get_host(host)}:{port}/" --data-binary @-'
            )
