...
use wiremock::{Match, Request, Respond, ResponseTemplate};

use crate::tests::{error::TestError, stubs::Stub};
...
impl StubMatcher {
    pub fn load_stubs<P: AsRef<Path>>(
        stub_directory: P,
        ...
        for entry in entries.filter_map(Result::ok) {
                    ...
                    .map_or(false, |ext| ext == "yaml" || ext == "yml")
            {
                match fs::read_to_string(&path) {
                    ...
                    Err(e) => {
                        eprintln!("Failed to read stub file {:?}: {}", path, e)
                    }
                }
            }
        }
...
impl Respond for StubMatcher {
    fn respond(&self, request: &Request) -> ResponseTemplate {
        self.stubs
            .iter()
            .find(|stub| stub.matches(request))
            .expect("No stub found")
            .response
            .clone()
            .into_response_template()
    }
}
