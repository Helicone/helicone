...
async fn trace_propagation() {
    ...
    config.telemetry.propagate = true;
    ...
    let request = Request::builder()
        ...
        .header(
            "traceparent",
            format!("00-{}-7a085853722dc6d2-00", request_id),
        )
        .body(request_body)
        .unwrap();
...
#[tokio::test]
#[serial_test::serial(default_mock)]
async fn trace_propagation_with_config() {
    let mut config = Config::test_default();
    ...
    let response = harness.call(request).await.unwrap();
    assert_eq!(response.status(), StatusCode::OK);
    assert_ne!(response.headers().get("x-request-id").unwrap(), request_id);
